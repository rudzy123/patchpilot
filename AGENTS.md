...
- Parser-host pending-queue size is selected at 0 by Accepted [ADR 0028](docs/adr/0028-osv-runtime-enablement-architecture-and-safety.md). The committed host constant remains `unavailable` until R4C/R5A (Phase A implementation).
- Duplicate JSON-key detection is not implemented. ADR 0028 Option B: residual risk for a later disabled canary; detection or explicit exception before activation (Phase A/R7).
- Operational runbook **outlines** live in ADR 0028. They are not live procedures (Phase A).
...
- Next checkpoint after Session 11 closure and ADR 0028 acceptance is R2 (production GCS listing executor). Session 11 remains zero-Finding. Session 11 acquisition foundation is **CLOSED**.
