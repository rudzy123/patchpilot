...
Last verified checkpoint: Session 13 Batch 2C-R (canary authorization adapter adversarial review; uncomposed; no CLI or provider contact).
Current session: Session 13.
Current checkpoint: Session 13 Batch 2C-R independently reviewed and hardened the uncommitted Batch 2C PostgreSQL adapters (`createOsvCanaryAuthorizationPersistence` in `@patchpilot/database`). Operator identity is insert-once with immutable replay. Revoked operators cannot receive new authorizations or consume unconsumed authorizations. Authorization ensure and consume use database time. Single-use consumption admits one CAS winner. Same-run replay is status reuse, not second execution. Different-run replay is rejected. Legal-decision and listing-review substitution fail closed. Terminal authorization state is immutable. Production composition does not construct the factory. Authentication, CLI, heartbeat, deadline, legal issuance, and provider contact remain later gates. Session 13 Batch 2D one-shot operator command boundary is next. No real provider contact has occurred.
...
- Session 12 remains zero-Finding. Session 13 Batch 2C-R remains zero-Finding. Generic Finding schema exists and is unused by intelligence import.
...
- Finding writes are not authorized in Session 12 or Session 13 Batch 2C-R. They remain blocked until an active authoritative catalog, package normalization, a reviewed ecosystem evaluator, a deterministic `affected` result, persisted immutable match evidence, [ADR 0026](docs/adr/0026-authoritative-match-evidence-and-finding-lifecycle.md) gates, tenant-isolation proof, and explicit Finding-write authorization are complete. Architectural gates are authoritative, not the assigned session number. The current roadmap places that work no earlier than Session 16.
