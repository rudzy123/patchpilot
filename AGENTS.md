...
Last verified checkpoint: Session 13 Batch 2A (instance-operator identity and canary authorization contracts; persistence unauthorized).
Current session: Session 13.
Current checkpoint: Session 13 Batch 2A defines framework-independent instance-operator identity and single-use OSV canary authorization contracts in `@patchpilot/vulnerability-intelligence`. Authentication, persistence, CLI, heartbeat, and provider contact remain later gates. Session 13 Batch 2B canary authorization persistence is next. No real provider contact has occurred. No retry executor, scheduler, or production runtime composition exists.
...
- Session 13 Batch 2A defines framework-independent canary-scoped instance-operator identity and single-use authorization contracts in `@patchpilot/vulnerability-intelligence` (`src/osv/canary-authorization/`). Authentication is contract-only and not implemented. Authorization persistence, CLI, heartbeat, deadline, and provider contact remain later gates. Halt release and synthetic verification grant no authority. Session 13 remains zero-Finding.
...
- Session 12 remains zero-Finding. Session 13 Batch 2A remains zero-Finding. Generic Finding schema exists and is unused by intelligence import.
...
- Finding writes are not authorized in Session 12 or Session 13 Batch 2A. They remain blocked until an active authoritative catalog, package normalization, a reviewed ecosystem evaluator, a deterministic `affected` result, persisted immutable match evidence, [ADR 0026](docs/adr/0026-authoritative-match-evidence-and-finding-lifecycle.md) gates, tenant-isolation proof, and explicit Finding-write authorization are complete. Architectural gates are authoritative, not the assigned session number. The current roadmap places that work no earlier than Session 16.
...
- Session 13: Batch 1-R accepted ADR 0029 canary architecture (execution unauthorized; no real provider contact). Batch 2A defines operator-identity and authorization contracts. Batch 2B canary authorization persistence is next, then CLI, heartbeat, deadline, runbooks, listing-only canary, body canary, and later catalog-activation work.
...
- Persistence intent documents Batch 2B tables (`osv_canary_instance_operator_identity`, `osv_canary_authorization`). Prisma is unchanged. No CLI, API, scheduler, job route, Outbox route, heartbeat, deadline timer, provider contact, activation, matching, or Findings.
- `INTELLIGENCE_OSV_ENABLED=true` remains rejected. Halt defaults true. Session 13 remains zero-Finding. Next checkpoint is Session 13 Batch 2B canary authorization persistence.
