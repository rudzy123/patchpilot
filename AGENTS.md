...
### Known Session 9 gaps (after Batch 4A)
...
- Public provider-status contracts and provider-neutral ports exist. Routes, adapters, and workers do not.
- The production KEV catalog body is not stored in the repository.
- KEV-first typed configuration exists. It does not start a scheduler or contact providers.
- OSV runtime remains disabled. `INTELLIGENCE_OSV_ENABLED=true` must fail configuration validation.
- No Session 9 Prisma migration, provider HTTP runtime, snapshot-storage adapter, complete KEV parser, worker-thread execution, worker sync jobs, API, or ZIP dependency.
- Final snapshot object-key layout remains deferred ([ADR 0021](docs/adr/0021-vulnerability-intelligence-import-foundation.md)).
...
- Existing required unique `osvId` is unchanged; provider-neutral Vulnerability identity is deferred ([OD-19](docs/architecture/open-decisions.md)).
- Specified `intelligence.sync_*` audit actions have command types only and are not emitted.
...
apps/
  ...
  api/                         # Fastify TypeScript API (health, auth, assets, SBOM upload)

  worker/                      # Node.js TypeScript workers (outbox relay + SBOM ingest processor)
packages/
  ...
  integrations/ # Object-storage S3 adapter and Redis ports
  logger/
  observability/
  policy-engine/               # empty boundary
  sbom/                        # vendored CycloneDX JSON schemas; worker-thread parser
  test-utils/
  vulnerability-intelligence/  # vendored CISA KEV JSON Schema; offline Ajv compile; parser-thread DTOs
  eslint-config/
  typescript-config/
docs/
  adr/
