...
- The 1 MiB snapshot ceiling is an object-storage admission limit. OD-8 provider HTTP retrieval byte policy remains unavailable. Do not treat Batch 5F as provider-retrieval readiness. The next checkpoint is Session 11 Batch 6A-P.
- No provider retrieval, synchronization, matching, Findings, or OSV enablement. Synthetic bytes only. `INTELLIGENCE_OSV_ENABLED=true` remains rejected. Session 11 remains zero-Finding. Session 12 remains zero-Finding.
- [ADR 0027](docs/adr/0027-osv-acquisition-persistence-and-catalog-activation.md) remains Proposed.
