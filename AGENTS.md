...
Last verified checkpoint: Session 13 Batch 3B-A-R (adversarially reviewed uncomposed listing-only provider-contact authorization PostgreSQL adapters; no production composition; no real provider contact).
Current session: Session 13.
Current checkpoint: Session 13 Batch 3B-A-R independently reviewed and hardened uncomposed `createOsvListingProviderContactAuthorizationPersistence` in `@patchpilot/database` against the frozen Batch 3B-P schema. Issuance validates the consumed listing-only source canary authorization, exact request and run, accepted provider-free preflight evidence, operator, legal, egress, deployment, and closed listing-only policy bindings, then inserts with database-owned `issued_at` and `expires_at = issued_at + 3600 seconds`. Insert requires current legal revalidation at database time. Identical issued replay is `already_applied` only when immutable fields including legal evidence-set identity, legal issued-at, and acknowledgement timestamps match. Immutable disagreement is `immutable_conflict`. Time-expired issued rows cannot be treated as idempotent reissue. Inspection is read-only and uses database time so `databaseNow >= expiresAt` is expired. Consumption is one compare-and-swap from `issued` with exact bindings and `CURRENT_TIMESTAMP < expires_at`; same-run replay is status only; different-run replay fails closed without revealing the other run; operator revocation of an issued grant is `operator_revoked`. Revocation is from `issued` only, uses database time, and is idempotent for the already-revoked issuer. There is no row-revision column; CAS uses issued state, bindings, and database time. Adapters perform no provider, lease, timer, activation, matching, or Finding work. Session 13 Batch 3B evaluation still fails closed with `persistence_required` because production composition does not construct these adapters or the evaluation factory. No public CLI, API, scheduler, or job route is added. Next checkpoint is Session 13 Batch 3B-R combined provider-contact authorization review. Batch 3C remains blocked until reviewed consumption composition exists. No real provider contact has occurred.
Current branch: `feat/osv-provider-canary`.

PatchPilot currently provides authentication, organization selection, asset inventory, SBOM upload and ingestion, local graph persistence, local CISA KEV synchronization, sanitized provider status, canonical CVE identity, a disabled synthetically verified OSV acquisition foundation, an uncomposed, adversarially reviewed Session 12 Batch 1 GCS listing-page HTTPS executor, Session 12 Batch 3 framework-independent listing pagination and two-pass inventory convergence contracts, an explicitly invoked Session 12 Batch 4 in-memory pagination and convergence service, the Session 12 Batch 4-R adversarial review of that service, Session 12 Batch 5 framework-independent durable synchronization job, lease, retry, parser-capacity, halt, cancellation, and operational-event contracts, Session 12 Batch 6 schema-only OSV runtime coordination persistence, Session 12 Batch 7 uncomposed PostgreSQL runtime-coordination adapters, the Session 12 Batch 7-R adversarial review of those adapters, Session 12 Batch 8 disabled runtime composition (`createOsvDisabledRuntimeSynchronization`), Session 12 Batch 9 typed acquisition halt and bounded operational observability, the Session 12 Batch 10 runtime-enablement foundation closure review, Session 13 Batch 2E-R adversarially reviewed uncomposed canary lease-heartbeat and monotonic-deadline controllers, Session 13 Batch 2F uncomposed executable canary preflight, Session 13 Batch 2F-R adversarial review of that preflight, Session 13 Batch 2-R combined operational-controls review, Session 13 Batch 3A uncomposed listing-only execution bridge against a scripted provider port (`createOsvCanaryListingOnlyExecutionBridge`), the Session 13 Batch 3A-R adversarial review of that bridge, Session 13 Batch 3B uncomposed listing-only provider-contact authorization evaluation (`createOsvListingProviderContactAuthorizationService`), and Session 13 Batch 3B-A uncomposed listing-only provider-contact authorization PostgreSQL adapters (`createOsvListingProviderContactAuthorizationPersistence`), adversarially reviewed in Session 13 Batch 3B-A-R. The pagination service, Batch 5 contracts, Batch 6 tables, Batch 7 adapters, Batch 8 composition, Batch 9 halt/observability controls, Batch 2E controllers, Batch 2F preflight, the one-shot command, the Batch 3A execution bridge, the Batch 3B provider-contact authorization service, the Batch 3B-P provider-contact authorization tables, and the Batch 3B-A persistence factory remain production-unreachable. Tests do not contact a provider. No real-provider capability exists.
...
- Finding writes are not authorized in Session 12 or Session 13 Batch 3B-A. They remain blocked until an active authoritative catalog, package normalization, a reviewed ecosystem evaluator, a deterministic `affected` result, persisted immutable match evidence, [ADR 0026](docs/adr/0026-authoritative-match-evidence-and-finding-lifecycle.md) gates, tenant-isolation proof, and explicit Finding-write authorization are complete. Architectural gates are authoritative, not the assigned session number. The current roadmap places that work no earlier than Session 16.
