...
### Known Session 9 gaps (after Batch 3B)
...
- No Session 9 contracts, Prisma migration, provider HTTP, snapshot object-key layout, complete KEV parser, worker-thread execution, worker sync jobs, API, or ZIP dependency.
...
- Specified `intelligence.sync_*` audit actions are design only and are not emitted.
...
packages/
  ...
  test-utils/
  vulnerability-intelligence/  # vendored CISA KEV JSON Schema; offline Ajv compile
  eslint-config/
  typescript-config/
docs/
  adr/
