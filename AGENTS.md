...
- Existing required unique `osvId` is unchanged. [ADR 0023](docs/adr/0023-provider-neutral-cve-identity.md) accepts canonical CVE identity as domain/ADR only; persistence and migration remain future. Full provider-neutral Vulnerability advisory identity remains open ([OD-19](docs/architecture/open-decisions.md)).
...
- Canonical CVE identity persistence and the forward-only identity migration remain future. Do not treat Batch 1B types as a shipped schema.
- KEV membership remains a global exploitation signal, not proof of tenant exposure. Read-only active-KEV derivation is not implemented in this batch.
- No API route, worker, Outbox event, permission, or web UI was added.
