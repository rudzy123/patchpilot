...
### Known Session 9 gaps (after Batch 4D)
...
- Public provider-status contracts and provider-neutral ports exist. HTTP routes, workers, and a scheduler loop do not.
...
- No provider HTTP runtime, snapshot-storage adapter, complete KEV parser, worker-thread execution, worker sync jobs, API, or ZIP dependency.
