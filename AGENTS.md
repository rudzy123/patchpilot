...
- Input payload representation remains deferred. Provider-object body byte limits remain `unavailable` (OD-8 / Batch 3B). Executable parser request construction and the parser port remain blocked until an approved input-byte limit exists.
- All parser resource limits (input/output bytes, depth, collection bounds, timeouts) are explicitly `unavailable`. No invented production defaults. No environment or caller override.
