...
### Known Session 9 gaps (after Batch 7B)
...
- The framework-independent CISA KEV synchronization service exists in `@patchpilot/domain`. It is not started by application boot, a scheduler loop, or a BullMQ Worker.
- Public provider-status contracts exist. HTTP routes, workers, Outbox relay mapping for `intelligence.sync.requested.v1`, and a scheduler loop do not.
...
- Restricted CISA KEV HTTPS transport and private intelligence snapshot storage exist in `@patchpilot/integrations`. Importing those modules does not contact CISA. No production worker wires them to the synchronization service yet.
- The production KEV catalog body is not stored in the repository.
- KEV-first typed configuration exists, including sync attempt and lease-renewal settings. It does not start a scheduler and does not contact providers by itself.
...
packages/
  ...
  domain/                      # Result/error taxonomy; persistence ports; CISA KEV sync service; no Prisma types
