...
Last verified checkpoint: Session 13 Batch 3B-P-R (listing-only provider-contact authorization persistence adversarially reviewed and frozen; no issuance or consumption adapter; no real provider contact).
Current session: Session 13.
Current checkpoint: Session 13 Batch 3B-P-R independently reviewed and hardened the uncommitted Batch 3B-P schema. Migration `20260909120000_osv_listing_provider_contact_authorization_persistence` (frozen SHA-256 `8e9a462e329733660b970adca64fcadce0431d65f342d15dbaecf08b91a80bbc`). Sixteen frozen migrations. All fifteen prior migrations remain byte-for-byte unchanged. Concrete corrections: `issued_at` after database now is rejected so unused TTL cannot be extended; `issued_at` cannot precede preflight `captured_at`; future preflight `captured_at`/`created_at` and future `consumed_at` are rejected; consumption rechecks an active operator, a still-consumed source canary authorization, a current legal revalidation boundary, and the bound preflight still attesting unauthorized provider contact; a consumed source cannot leave consumed while a child grant is issued or `consumed_for_listing_execution`; insert matches source provider, prefix, family, policy, and listing budget. Global instance-owned tables `osv_canary_provider_free_preflight_attestation` and `osv_listing_provider_contact_authorization` pin one consumed listing-only source canary authorization, one request, one run, one accepted provider-free preflight attestation, and closed listing-only policy, legal, egress, deployment, controller, runbook, reviewer, and retention identifiers. Phase is listing_only only. Body retrieval, parser, retry, activation, matching, and Finding writes remain prohibited. Database time owns issuance, expiration, consumption, revocation, and terminalization. DELETE is forbidden. At most one provider-contact authorization exists per source canary authorization, request, run, and preflight attestation. Schema existence does not issue or consume an authorization. Session 13 Batch 3B evaluation still fails closed with `persistence_required` because no issuance adapter exists. Production composition does not construct the evaluation factory. No public CLI, API, scheduler, or job route is added. Next checkpoint is durable listing-only provider-contact authorization issuance. Batch 3C remains blocked until durable issuance exists. No real provider contact has occurred.
Current branch: `feat/osv-provider-canary`.

PatchPilot currently provides authentication, organization selection, asset inventory, SBOM upload and ingestion, local graph persistence, local CISA KEV synchronization, sanitized provider status, canonical CVE identity, a disabled synthetically verified OSV acquisition foundation, an uncomposed, adversarially reviewed Session 12 Batch 1 GCS listing-page HTTPS executor, Session 12 Batch 3 framework-independent listing pagination and two-pass inventory convergence contracts, an explicitly invoked Session 12 Batch 4 in-memory pagination and convergence service, the Session 12 Batch 4-R adversarial review of that service, Session 12 Batch 5 framework-independent durable synchronization job, lease, retry, parser-capacity, halt, cancellation, and operational-event contracts, Session 12 Batch 6 schema-only OSV runtime coordination persistence, Session 12 Batch 7 uncomposed PostgreSQL runtime-coordination adapters, the Session 12 Batch 7-R adversarial review of those adapters, Session 12 Batch 8 disabled runtime composition (`createOsvDisabledRuntimeSynchronization`), Session 12 Batch 9 typed acquisition halt and bounded operational observability, the Session 12 Batch 10 runtime-enablement foundation closure review, Session 13 Batch 2E-R adversarially reviewed uncomposed canary lease-heartbeat and monotonic-deadline controllers, Session 13 Batch 2F uncomposed executable canary preflight, Session 13 Batch 2F-R adversarial review of that preflight, Session 13 Batch 2-R combined operational-controls review, Session 13 Batch 3A uncomposed listing-only execution bridge against a scripted provider port (`createOsvCanaryListingOnlyExecutionBridge`), the Session 13 Batch 3A-R adversarial review of that bridge, and Session 13 Batch 3B uncomposed listing-only provider-contact authorization evaluation (`createOsvListingProviderContactAuthorizationService`). The pagination service, Batch 5 contracts, Batch 6 tables, Batch 7 adapters, Batch 8 composition, Batch 9 halt/observability controls, Batch 2E controllers, Batch 2F preflight, the one-shot command, the Batch 3A execution bridge, the Batch 3B provider-contact authorization service, and the Batch 3B-P provider-contact authorization tables remain production-unreachable. Tests do not contact a provider. No real-provider capability exists.
...
- Session 12 Batch 3 pagination contracts, Session 12 Batch 4 `createOsvListingPaginationService`, the Session 12 Batch 4-R hardened pagination service, Session 12 Batch 5 runtime-coordination contracts, Session 12 Batch 7 `createOsvRuntimeCoordinationPersistence`, Session 12 Batch 8 `createOsvDisabledRuntimeSynchronization`, Session 13 Batch 2C `createOsvCanaryAuthorizationPersistence`, Session 13 Batch 2D `createOsvCanaryOneShotCommandService`, Session 13 Batch 2E `createOsvCanaryLeaseHeartbeatController`, Session 13 Batch 2E `createOsvCanaryDeadlineController`, Session 13 Batch 2F `createOsvCanaryPreflightService`, Session 13 Batch 2F `createOsvCanaryPreflightReadiness`, Session 13 Batch 3A `createOsvCanaryListingOnlyExecutionBridge`, and Session 13 Batch 3B `createOsvListingProviderContactAuthorizationService` are exported and are not imported by worker, API, scheduler, queue, health, seed, or migration composition. Session 12 Batch 6 tables exist and are unused by production runtime. Session 13 Batch 2B canary authorization tables exist and are unused by production runtime. Session 13 Batch 3B-P provider-contact authorization tables exist and are unused by production runtime. The future job type `intelligence.osv.sync` is not registered in production worker routing.
...
- Durable OSV job registration, retry execution, scheduler wiring, real-provider canary execution, catalog activation, matching, and Finding writes. Token-cycle detection and A/B convergence execute only inside the uncomposed Batch 4 in-memory service, the disabled Batch 8 composition, and the uncomposed Batch 3A scripted listing-only bridge. Batch 5 defines those coordination contracts. Batch 6 persists the schema. Batch 7 persists authority through uncomposed adapters without executing work. Batch 7-R reviewed those adapters. Batch 8 composes them behind a test-only verification factory that is not a public package export. Batch 8-R adversarially reviewed that composition. Batch 9 added typed halt configuration and bounded operational observability without production composition. Batch 9-R adversarially reviewed that halt and observability work. Session 12 Batch 10 closed the runtime-enablement foundation. Session 13 Batch 1-R accepted canary architecture only. Session 13 Batch 2A added identity and authorization contracts only. Session 13 Batch 2B added schema-only persistence. Session 13 Batch 2C adds uncomposed authorization adapters. Session 13 Batch 2D adds an uncomposed one-shot command service that authenticates through an injected port, consumes one existing authorization, and stops at `authorized_preflight_required`. Session 13 Batch 2D-R adversarially reviewed that command. Session 13 Batch 2E adds uncomposed heartbeat and deadline controllers that do not acquire a lease or contact a provider. Session 13 Batch 2E-R adversarially reviewed those controllers. Session 13 Batch 2F adds an uncomposed executable preflight that does not authorize provider contact. Session 13 Batch 2F-R independently reviewed that preflight. Session 13 Batch 2-R independently reviewed the combined operational-control chain. Session 13 Batch 3A adds an uncomposed listing-only execution bridge against a scripted provider port only. Session 13 Batch 3A-R independently reviewed that bridge. Session 13 Batch 3B adds an uncomposed listing-only provider-contact authorization evaluation that fails closed with `persistence_required` after other gates pass and does not mint in-memory authority. Session 13 Batch 3B-P adds schema-only provider-contact authorization persistence without issuance or consumption adapters. Durable issuance adapters are Session 13 Batch 3B-P-R then later batches. Production operator authentication, public CLI, legal issuance, real-provider capability, Batch 3C listing-only provider contact, and Finding writes remain later gates.
...
- Finding writes are not authorized in Session 12 or Session 13 Batch 3B-P. They remain blocked until an active authoritative catalog, package normalization, a reviewed ecosystem evaluator, a deterministic `affected` result, persisted immutable match evidence, [ADR 0026](docs/adr/0026-authoritative-match-evidence-and-finding-lifecycle.md) gates, tenant-isolation proof, and explicit Finding-write authorization are complete. Architectural gates are authoritative, not the assigned session number. The current roadmap places that work no earlier than Session 16.
...
- No issuance, consumption, revocation, or CLI adapter in this batch. No scheduler, BackgroundJob route, Outbox route, or production composition. Next checkpoint is durable issuance. `INTELLIGENCE_OSV_ENABLED=true` remains rejected. Halt defaults true. Session 13 remains zero-Finding.
