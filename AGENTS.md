...
- Durable persistence remains required. Schema work cannot begin until the encryption-policy checkpoint closes ciphertext version, nonce or equivalent, associated-data binding, key ownership, and rotation. Plaintext, reversible encoding, base64-as-encryption, static keys, and keys stored beside ciphertext are forbidden.
...
- Batch 3C authority remains consumed and unreusable. Batch 4-P remains blocked. Production composition does not import the module. `INTELLIGENCE_OSV_ENABLED=true` remains rejected. Halt defaults true. Session 13 remains zero-Finding. Next checkpoint: encryption-policy checkpoint before Session 13 Batch 3D-S.
