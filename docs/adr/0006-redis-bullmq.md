...
Use **Redis** and **BullMQ** as the job transport **after** the transactional outbox. Domain and use cases do not import BullMQ or Redis. Workers (including the relay) depend on queue adapters. `apps/api` handlers must not publish jobs to Redis. Redis is not a second source of truth. Production Redis is authenticated and not browser-exposed. Sessions are **not** stored in Redis ([ADR 0019](0019-local-password-sessions.md); Redis may rate-limit login only).
