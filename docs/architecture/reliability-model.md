...
Restore both stores together. Degraded mode during provider outages: last intel snapshots, freshness visible, no fake "all clear." Future protected listing-evidence ciphertext may appear in PostgreSQL backups. Raw encryption keys must not. Restoring the database without the required instance key capability leaves those protected values unavailable. Cryptographic erasure of one evidence set does not immediately remove ciphertext from backups. Session 13 Batch 3D-S / 3D-S-R stores restricted envelopes in schema only. Session 13 Batch 3D-C encrypts synthetic identities in process and does not persist envelopes. Session 13 Batch 3D-C-R independently reviewed that capability.
