...
Session 9 KEV scheduler, bounded provider retry, PostgreSQL retry reconciliation, staging, and atomic activation are implemented. Still unimplemented: object-storage orphan cleanup (including intelligence final-snapshot orphans), stale `running` job sweeps, production OSV runtime (the Session 12 runtime-enablement foundation exists and remains uncomposed; Session 13 Batch 2E-R canary heartbeat and deadline controllers exist, are adversarially reviewed, remain uncomposed, and do not register production timers; Session 13 Batch 2F executable preflight exists, remains uncomposed, does not contact a provider, and does not authorize provider contact; Session 13 Batch 2F-R independently reviewed that preflight; Session 13 Batch 2-R independently reviewed the combined operational-control chain; Session 13 Batch 3A listing-only execution bridge exists, remains uncomposed, supports scripted provider execution only, was independently reviewed in Session 13 Batch 3A-R, and does not contact a real provider; Session 13 Batch 3B listing-only provider-contact authorization evaluation exists, remains uncomposed, fails closed with `persistence_required` after other gates pass, and does not start execution or contact a provider; Session 13 Batch 3B-P listing-only provider-contact authorization tables exist, remain unused by production runtime, and do not issue or consume on schema existence; Session 13 Batch 3B-P-R independently reviewed and froze that schema; Session 13 Batch 3B-A uncomposed issuance, inspection, consumption, and revocation adapters exist, were independently reviewed in Session 13 Batch 3B-A-R, remain unused by production runtime, and do not contact a provider; Session 13 Batch 3B-R independently reviewed the complete authorization chain without composing production runtime or contacting a provider; Session 13 Batch 3C-Auth listing-canary execution-authorization exists, remains uncomposed, prepares one private one-use listing-attempt permit, was independently reviewed in Session 13 Batch 3C-Auth-R, and does not contact a provider on import; Session 13 Batch 3C bounded listing canary exists, remains uncomposed, executed exactly one operator-controlled listing-only real-provider HTTPS request (`listing_canary_one_page_classified`; halt restored; lease released), was independently reviewed in Session 13 Batch 3C-R (candidate-selection evidence unavailable; Batch 4-P blocked), is not registered in worker or API startup, and does not enable general OSV runtime; Session 13 Batch 3D-P-R independently reviewed that protected listing-observation evidence policy; it remains uncomposed, is provider-free, does not persist real observations, and does not authorize another listing request; Session 13 Batch 3D-E defines encryption policy only, does not encrypt or persist, and was independently reviewed in Session 13 Batch 3D-E-R so schema design may proceed without a plaintext protected-key column), expired **RiskAcceptance**, and expired **manual_override** calculations. Delivery remains at-least-once; PatchPilot does not claim exactly-once. Runbooks: [docs/runbooks/](../runbooks/README.md).
...
Restore both stores together. Degraded mode during provider outages: last intel snapshots, freshness visible, no fake "all clear." Future protected listing-evidence ciphertext may appear in PostgreSQL backups. Raw encryption keys must not. Restoring the database without the required instance key capability leaves those protected values unavailable. Cryptographic erasure of one evidence set does not immediately remove ciphertext from backups. Session 13 Batch 3D-S / 3D-S-R stores restricted envelopes. Session 13 Batch 3D-C encrypts synthetic identities in process. Session 13 Batch 3D-C-R independently reviewed that capability. Session 13 Batch 3D-A persists those envelopes through uncomposed adapters; Session 13 Batch 3D-A-R independently reviewed those adapters; production composition does not construct the factory. Session 13 Batch 3D-Auth does not persist real protected evidence and does not construct the persistence factory. Session 13 Batch 3D-Auth-R independently reviewed that authorization.
