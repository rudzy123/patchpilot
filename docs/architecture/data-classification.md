...
| Protected OSV listing-evidence ciphertext, nonce, and authentication tag | Restricted | Session 13 Batch 3D-S / 3D-S-R stores these only as restricted `nonce_12 \|\| ciphertext \|\| tag_16` BYTEA; Session 13 Batch 3D-C constructs matching in-memory envelopes with Node.js 24 AES-256-GCM and does not persist them; Session 13 Batch 3D-C-R independently reviewed that capability; not public JSON, logs, events, metrics, traces, errors, or APIs; future public-read adapters must omit envelope columns; database access is not decryption authority; ciphertext is not harmless public data |
