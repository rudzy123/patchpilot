...
- Versions are monotonic integers per key. Published **RiskPolicy.definition**, identity, and `publishedAt` are immutable. Published rows cannot be deleted. The only allowed published-status change is `published` → `retired`.
