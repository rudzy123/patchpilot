...
- Future protected listing-evidence ciphertext may appear in PostgreSQL backups. Raw listing-evidence encryption keys must not. Restoring the database without the required key capability leaves protected object identities unavailable. Restoring old database state must not reactivate destroyed keys. Backup operators must not receive general decryption authority merely because they control backups. Cryptographic erasure does not immediately remove ciphertext from backups. Session 13 Batch 3D-S / 3D-S-R schema stores restricted envelopes. Session 13 Batch 3D-C implements uncomposed AES-256-GCM with operator-supplied runtime keys unused by startup; no encryption is persisted. Session 13 Batch 3D-C-R independently reviewed that capability. Session 13 Batch 3D-E-R treats recorded purge evidence as authoritative after restore even if keys remain available.
