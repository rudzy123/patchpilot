...
| OSV first-provider canary | `crates.io/` + `rustsec_advisory_database`; exact numeric caps; no activation | ADR 0028 §15. [ADR 0029](../adr/0029-first-real-provider-osv-canary-authorization-and-safety.md) Accepted. Execution remains blocked on legal revalidation, configured operator attestation, heartbeat, deadline, runbooks, and egress proof. Acceptance does not authorize provider contact. |
...
| OSV first-provider canary execution | Live `crates.io/` / RUSTSEC contact | Policy is Accepted as provisional technical candidate. ADR 0029 is Accepted architecture. Session 13 Batch 2A defines identity and authorization contracts. Session 13 Batch 2B adds schema-only persistence. Session 13 Batch 2B-R independently reviewed and froze that schema. Session 13 Batch 2C adds uncomposed authorization adapters. Session 13 Batch 2C-R independently reviewed those adapters. Session 13 Batch 2D adds an uncomposed one-shot command that stops at `authorized_preflight_required`. Session 13 Batch 2D-R independently reviewed that command. Session 13 Batch 2E adds uncomposed heartbeat and deadline controllers that do not acquire a lease or contact a provider. Session 13 Batch 2E-R independently reviewed those controllers. Session 13 Batch 2F adds uncomposed executable preflight whose success is `canary_execution_preflight_passed_provider_contact_not_authorized`. Session 13 Batch 2F-R independently reviewed that preflight. Session 13 Batch 2-R independently reviewed the combined operational-control chain. Session 13 Batch 3A adds an uncomposed listing-only execution bridge against a scripted provider port. Session 13 Batch 3A-R independently reviewed that bridge. Session 13 Batch 3B adds uncomposed listing-only provider-contact authorization evaluation; after other gates pass it fails closed with `persistence_required` and does not mint in-memory authority. Existing canary authorization cannot represent this without phase and lifecycle ambiguity. Session 13 Batch 3B-P adds schema-only distinct listing-only provider-contact authorization persistence (`20260909120000_osv_listing_provider_contact_authorization_persistence`, frozen SHA-256 `8e9a462e329733660b970adca64fcadce0431d65f342d15dbaecf08b91a80bbc`). Sixteen frozen migrations. Session 13 Batch 3B-P-R independently reviewed and froze that schema. Session 13 Batch 3B-A implements uncomposed listing-only provider-contact authorization issuance, inspection, consumption, and revocation adapters. Session 13 Batch 3B-A-R independently reviewed those adapters. Session 13 Batch 3B-R independently reviewed the complete authorization chain. Session 13 Batch 3C-Auth implements uncomposed listing-canary execution authorization (`createOsvListingCanaryExecutionAuthorizationService`) that prepares one private one-use listing-attempt permit and does not contact a provider. Session 13 Batch 3C-Auth-R independently reviewed that boundary. Production composition does not construct the factory. Successful consumption does not authorize provider contact. Real-provider capability does not exist. Not legally authorized for live contact. Legal revalidation outstanding. Canary-scoped operator identity adapters, the command service, the controllers, preflight, the scripted execution bridge, the provider-contact authorization service, the provider-contact persistence adapters, and the listing-canary execution-authorization service exist and are unused by production runtime; OD-10 remains open for general console. Heartbeat remains required before either phase executes. | Session 13 Batch 3C first bounded real-provider listing canary remains later. No activation. Listing-only before body. Aggregate body bytes ≤ 16777216. Acceptance of ADR 0029 does not authorize provider contact. Batch 3A through Batch 3C-Auth-R do not authorize real provider contact. |
