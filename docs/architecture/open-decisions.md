...
| OSV first-provider canary execution | Live `crates.io/` / RUSTSEC contact | Policy is Accepted as provisional technical candidate. ADR 0029 is Accepted architecture. Session 13 Batch 3C executed one operator-controlled listing-only real-provider GCS listing request (`listing_canary_one_page_classified`; one HTTPS listing request; continuation token unused; halt restored; lease released). Session 13 Batch 3C-R independently reviewed that evidence. Candidate-selection metadata is unavailable; Batch 4-P is blocked. Session 13 Batch 3D-P defines protected listing-observation evidence requirements for a future separately authorized listing and does not authorize another request. Session 13 Batch 3D-P-R independently reviewed that policy. Body retrieval remains blocked on legal revalidation and on a separately authorized future listing that retains protected observation metadata. Canary-scoped operator identity adapters, the command service, the controllers, preflight, the scripted execution bridge, the provider-contact authorization service, the provider-contact persistence adapters, the listing-canary execution-authorization service, the bounded listing-canary service, and the Batch 3D-P evidence contracts exist and are unused by production runtime; OD-10 remains open for general console. Heartbeat remains required before either phase executes. | Do not retry the listing request. Do not request a second page. No activation. Listing-only before body. Aggregate body bytes ≤ 16777216. Acceptance of ADR 0029 does not authorize general OSV enablement. |
| OSV protected listing-observation evidence persistence | Durable store for listing metadata required by candidate selection | Session 13 Batch 3D-P selects Option A `durable_protected_persistence_required` because the Batch 3C-R ephemeral database discarded candidate metadata. Session 13 Batch 3D-P-R independently reviewed that decision. Future tables are named only (`osv_listing_observation_evidence_set`, `osv_listing_observation_evidence`, `osv_listing_observation_evidence_purge`). Prisma and migrations are unchanged. Encryption at rest is a schema-blocking prerequisite; no plaintext protected-key column may be created, including temporarily. Next checkpoint is an encryption-policy checkpoint before Session 13 Batch 3D-S. | Do not persist raw listing responses or continuation tokens. Protected object keys are Restricted. Do not expose a bare object-key digest. Do not treat candidate eligibility as body authority. |
