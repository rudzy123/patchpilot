...
| OD-8 | Exact outbound rate-limit and import size numbers | KEV limits are provisionally in `@patchpilot/config`. Session 11 Batch 3C compiles OSV GCS listing `maxResults` (`1000`) and listing-page byte cap (`1,048,576`) in `@patchpilot/vulnerability-intelligence`. Continuation-token byte bounds, provider-object body limits, parser/worker numeric limits, and ZIP remain unselected. Session 11 Batch 4B records those parser limits as explicit `unavailable` on `osv_advisory_parser_resource_policy_v1` and blocks executable parse authorization until an approved input-byte limit exists. Conditional GET is not the KEV protocol. | Use the typed KEV bounds. Use the compiled GCS listing protocol constants for listing construction and page parsing only. Keep Batch 4B parser envelopes metadata-only. Keep `INTELLIGENCE_OSV_ENABLED=false`. Do not treat earlier prose (30 req/min, 5 MiB) as provider SLAs. Do not encode dated or discussion-only OSV archive sizes as configuration or as first-implementation authorization. |
