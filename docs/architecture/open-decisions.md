...
| OSV first-provider canary execution | Live `crates.io/` / RUSTSEC contact | Policy is Accepted as provisional technical candidate. ADR 0029 is Accepted architecture. Session 13 Batch 3C executed one operator-controlled listing-only real-provider GCS listing request (`listing_canary_one_page_classified`; one HTTPS listing request; continuation token unused; halt restored; lease released). Session 13 Batch 3C-R independently reviewed that evidence. Session 13 Batch 3D executed a separately authorized one-page protected-evidence listing (`protected_evidence_listing_canary_one_page_persisted`; one HTTPS listing request; 1000 AES-256-GCM-protected observations atomically persisted as one `review_pending` evidence set; continuation token unused; halt restored; lease released). Do not retry either request. Do not request a second page. Candidate selection remains unauthorized; Batch 4-P is blocked. Session 13 Batch 3D-R independently reviewed the protected evidence as sufficient for deterministic candidate evaluation. Body retrieval remains blocked on legal revalidation and on separately authorized candidate selection. Canary factories and operator scripts exist and are unused by production runtime; OD-10 remains open for general console. Heartbeat remains required before either phase executes. | Do not retry the listing request. Do not request a second page. No activation. Listing-only before body. Aggregate body bytes ≤ 16777216. Acceptance of ADR 0029 does not authorize general OSV enablement. |
| OSV protected listing-observation evidence persistence | Durable store for listing metadata required by candidate selection | Session 13 Batch 3D-P selects Option A `durable_protected_persistence_required`. Session 13 Batch 3D persisted one operator-controlled evidence set of 1000 AES-256-GCM envelopes in `review_pending` through uncomposed `createOsvListingObservationEvidencePersistence`. Public inspection omits envelope BYTEA and opaque key alias. No plaintext provider identity entered Prisma. Raw listing responses and continuation tokens were not persisted. Production composition does not construct the factory. Session 13 Batch 3D-R independently reviewed this evidence. Candidate selection remains unauthorized. Batch 4-P remains blocked. | Do not persist raw listing responses or continuation tokens. Protected object keys, ciphertext, nonce, tag, and complete key identifiers are Restricted. Do not expose a bare object-key digest. Do not treat candidate eligibility as body authority. Do not destroy the shared instance key as per-set purge. |
