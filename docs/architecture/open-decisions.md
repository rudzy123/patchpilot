...
| OD-10 | Instance operator identity | How a self-hosted admin authenticates separately from organization membership. | A config-gated bootstrap user that can manage **IntelligenceSource** rows and shared catalogs only. No cross-organization read of tenant evidence. A bypass ADR is required before any cross-org operator console. Session 13 Batch 2A defines canary-scoped actor kind `instance_operator` with a non-tenant attestation UUID (`osv_canary_instance_operator_identity_v1`) as a framework-independent contract. Session 13 Batch 2B persists that canary-scoped identity as schema only; Session 13 Batch 2B-R independently reviewed that schema. Session 13 Batch 2C adds uncomposed persistence adapters. Session 13 Batch 2C-R independently reviewed those adapters. Authentication is not implemented. OD-10 remains open. Do not use a tenant User id. |
...
| OSV first-provider canary execution | Live `crates.io/` / RUSTSEC contact | Policy is Accepted as provisional technical candidate. ADR 0029 is Accepted architecture. Session 13 Batch 2A defines identity and authorization contracts. Session 13 Batch 2B adds schema-only persistence. Session 13 Batch 2B-R independently reviewed and froze that schema. Session 13 Batch 2C adds uncomposed authorization adapters. Session 13 Batch 2C-R independently reviewed those adapters. Not legally authorized. Legal revalidation outstanding. Canary-scoped operator identity adapters exist and are unused by production runtime; OD-10 remains open for general console. Heartbeat controller required before either phase. | Session 13 after Batch 2 operational controls, legal revalidation, configured attestation, heartbeat, runbooks, and egress proof. No activation. Listing-only before body. Aggregate body bytes ≤ 16777216. Acceptance of ADR 0029 does not authorize provider contact. Batch 2C-R does not authorize provider contact. |
