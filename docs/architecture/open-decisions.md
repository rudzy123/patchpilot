...
| OD-8 OSV generation-bound retrieval byte limit | Maximum received bytes for one exact-generation OSV provider object | **CLOSED** (Session 11 Batch 6A-P). PatchPilot v1 ceiling: 1,048,576 bytes (1 MiB) for one generation-bound advisory body. This is security policy, not a provider guarantee. Oversize records fail closed and must prevent later generation activation. Silent omission from a complete generation is forbidden. Streaming incremental SHA-256 terminates immediately on first byte above limit. Policy identifier: `osv_generation_bound_retrieval_policy_v1`. Implementation: `@patchpilot/vulnerability-intelligence` (policy), `@patchpilot/integrations` (HTTPS adapter). Frozen in Session 11; changes require new forward-only migration and policy version. |
| OD-8 OSV archive runtime limits | Compressed/expanded `all.zip` operator download authorization | **Deferred and not authorized.** [ADR 0024](../adr/0024-authoritative-affected-version-source-and-osv-acquisition.md) does not approve `all.zip` as the first implementation. ZIP remains absent. OSV runtime remains disabled. |
