...
| OD-8 | Exact outbound rate-limit and import size numbers | KEV limits are provisionally in `@patchpilot/config`. Session 11 Batch 3C compiles OSV GCS listing `maxResults` (`1000`) and listing-page byte cap (`1,048,576`) in `@patchpilot/vulnerability-intelligence`. Session 11 Batch 4B-P closes **parser** numeric limits as PatchPilot policy on `osv_advisory_parser_resource_policy_v1` (not provider maxima). Continuation-token byte bounds, **provider-object body** retrieval limits, and ZIP remain unselected. Conditional GET is not the KEV protocol. | Use the typed KEV bounds. Use the compiled GCS listing protocol constants for listing construction and page parsing only. Use parser resource policy v1 only for locally supplied bounded bytes and synthetic Batch 4C tests. Keep provider-object retrieval deferred. Keep `INTELLIGENCE_OSV_ENABLED=false`. Do not treat earlier prose (30 req/min, 5 MiB) as provider SLAs. Do not encode dated or discussion-only OSV archive sizes as configuration or as first-implementation authorization. |
