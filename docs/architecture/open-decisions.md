...
| OSV retry implementation | Durable job-layer retries | Session 12 Batch 5 defines `osv_runtime_retry_policy_v1` (three total attempts including the initial attempt; parser timeout max 2; bounded full jitter; HTTP 429 Retry-After capped at 30 s). Session 12 Batch 6 persists immutable stage attempts. No retry executor exists. | Adapters remain one-attempt. Batch 7 implements persistence adapters without executing retries. |
| OSV scheduler and job implementation | `intelligence.osv.sync` and lease row | Session 12 Batch 5 defines the job payload, shared catalog-scope lease, holder/revision/fencing model, and database-time expiry as contracts. Session 12 Batch 6 adds schema-only tables (`osv_runtime_synchronization_request`, `osv_runtime_synchronization_run`, `osv_runtime_lease_projection`, `osv_runtime_stage_attempt`; migration `20260907120000_osv_runtime_coordination_persistence`, frozen SHA-256 `7017b1c4b1d4bcae8bed4bdd0eb43559c0c89fce5b3636e0e889b276013cc3a6`). Session 12 Batch 6-R forbids lease-row deletion so fencing cannot reset. No job type is registered. No lease is acquired. Reserved Outbox `intelligence.osv.sync.requested.v1` is deferred. | Later adapters. Do not reuse KEV jobs. Lease fencing uses database time. Raw holder tokens are not stored. |
