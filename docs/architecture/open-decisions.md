...
| OSV production GCS listing execution | HTTPS listing adapter, inventory convergence, and provider contact | Session 11 compiles listing requests and parses supplied page bytes only. No executor exists. | Do not contact `storage.googleapis.com` until Runtime Enablement Phase R1 selects ownership, TLS, redirects, page bounds, and a canary. |
| OSV listing pagination and token-cycle protection | Page-token confidentiality, repetition, and cycle detection | Continuation-token byte bounds remain unselected. Cycle detection is not implemented. | Reject runtime listing until Phase R1 closes token bounds, cycle detection, maximum pages, and maximum listing bytes. |
| OSV parser-host pending capacity | Isolated parser pending-queue size | Occupancy is 1. Pending-queue size remains `unavailable`. Disabled orchestration pending work (32 metadata-only items) is a different control. | Runtime composition stays `blocked_pending_queue_size_unapproved` until Phase R1 selects an exact pending size or proves concurrency 1 suffices. |
| OSV retry and backoff policy | Automatic redispatch after orchestration-retryable retrieval failures | Adapter and orchestrator record retry disposition only. No retry loop, backoff, or Retry-After execution exists. | Phase R1 must close attempts, backoff, jitter, exhaustion, and quarantine before Phase B implements retries. |
| OSV scheduler and durable-job architecture | Job type, payload, lease, cadence, crash resume | No OSV BackgroundJob type, Outbox event, or BullMQ job exists. `intelligence.sync` remains KEV-only. | Phase R1 must assign ownership and payload bounds. Do not reuse KEV jobs as silent OSV carriers. |
| OSV production observability | Operator-safe metrics, logs, and traces | Confidential failure taxonomy exists in adapters. Production OSV telemetry contracts are unselected. | Phase R1 must map bounded signals without provider keys, URLs, bodies, or tenant data. |
| OSV cleanup and retention | Orphan reconciliation and referenced-evidence protection | Cleanup eligibility exists. No production cleanup executor or retention deletion exists. | Phase R1/B must keep attached and referenced objects out of executable cleanup. |
| OSV first-provider canary | First live-provider contact under a disabled catalog | No canary policy is executed. Tests use synthetic bytes and injected seams only. | Phase C is the earliest live-provider contact. It must not activate a catalog. |
| OSV activation authorization | Who may replace the active pointer, and when | Adapter `activateReadyGeneration` exists and is never invoked by disabled orchestration. | Phase D requires explicit authorization after canary. `ready_for_activation` is not `active`. |
| OSV duplicate JSON-key disposition | Last-key-wins `secure-json-parse` limitation | Detection is not implemented. Adversarial tests show it cannot bypass identity or eligibility gates. | Resolve, explicitly accept, or keep enablement blocked before OSV runtime. |
| OSV kill-switch and rollback | Emergency disablement and catalog rollback | Configuration rejects `INTELLIGENCE_OSV_ENABLED=true`. No runtime kill switch or activation rollback runbook exists. | Phase R1 must define disablement, rollback of an active pointer, and operator evidence. |
