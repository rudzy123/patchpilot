...
| OSV listing executor implementation | HTTPS listing adapter against ADR 0028 §1 | Session 12 Batch 1 implements `createOsvGcsListingHttpsAdapter`. Session 12 Batch 2 adversarially reviewed and hardened single-page transport. Session 12 Batch 4 threads exact `responseByteCount` on the one-page success result. Uncomposed and runtime-unreachable. | Do not contact `storage.googleapis.com` from production composition. Pagination service remains uncomposed. |
| OSV pagination implementation | Token-cycle controls and prefix-pass persistence | Session 12 Batch 3 defines framework-independent pagination and convergence contracts. Session 12 Batch 4 implements the bounded in-memory page loop, A/B passes, and crash restart from page one through an injected one-page listing port. Session 12 Batch 4-R adversarially reviewed and hardened atomic admission, constructed transport-success admission, hung-port cancellation, and token-scope isolation. Raw tokens and token digests remain in memory only. The service is explicitly invoked and production-unreachable. | Durable job persistence, inventory persistence coordination, and production six-prefix listing remain later. Do not treat Batch 4, Batch 4-R, or Batch 5 as production synchronization. |
...
| OSV retry implementation | Durable job-layer retries | Session 12 Batch 5 defines `osv_runtime_retry_policy_v1` (three total attempts including the initial attempt; parser timeout max 2; bounded full jitter; HTTP 429 Retry-After capped at 30 s). Session 12 Batch 6 persists immutable stage attempts. Session 12 Batch 7 inspects retry eligibility and records exhaustion without executing retries. Session 12 Batch 7-R keeps inspection non-authoritative and reservation transactional. Session 12 Batch 8 records one-attempt disposition and does not execute retries. Session 12 Batch 8-R confirmed adapter-level retry remains absent. No retry executor exists. | Adapters remain one-attempt. Execution remains later. |
| OSV scheduler and job implementation | `intelligence.osv.sync` and lease row | Session 12 Batch 5 defines the job payload, shared catalog-scope lease, holder/revision/fencing model, and database-time expiry as contracts. Session 12 Batch 6 adds schema-only tables (`osv_runtime_synchronization_request`, `osv_runtime_synchronization_run`, `osv_runtime_lease_projection`, `osv_runtime_stage_attempt`; migration `20260907120000_osv_runtime_coordination_persistence`, frozen SHA-256 `7017b1c4b1d4bcae8bed4bdd0eb43559c0c89fce5b3636e0e889b276013cc3a6`). Session 12 Batch 6-R forbids lease-row deletion so fencing cannot reset. Session 12 Batch 7 implements uncomposed PostgreSQL adapters for those tables. Session 12 Batch 7-R reviewed those adapters. Session 12 Batch 8 composes them through an explicitly constructed disabled factory that production startup does not import. Session 12 Batch 8-R confirmed the verification factory is not a public package export. No job type is registered. Reserved Outbox `intelligence.osv.sync.requested.v1` is deferred. | Do not reuse KEV jobs. Lease fencing uses database time. Raw holder tokens are not stored. Production composition remains gated. |
| OSV observability implementation | Bounded events and metrics | Session 12 Batch 5 defines closed operational-event contracts. No emitter exists. | Later runtime batch. Preserve the prohibition list. |
| OSV kill-switch variable | `INTELLIGENCE_OSV_ACQUISITION_HALT` | Session 12 Batch 9 adds typed configuration. Session 12 Batch 9-R reviewed default halt, own-property parsing, and observation-only halt ports. Default, missing, and empty values are halted. Explicit `false` releases halt only. Malformed values fail validation. Enablement remains rejected and does not clear halt. Environment refresh is a process snapshot. | Session 12 Batch 10 reviews runtime closure. Production composition, scheduler, and provider contact remain later. |
