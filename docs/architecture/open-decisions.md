...
| OSV listing executor implementation | HTTPS listing adapter against ADR 0028 §1 | Session 12 Batch 1 implements `createOsvGcsListingHttpsAdapter`. Session 12 Batch 2 adversarially reviewed and hardened single-page transport. Uncomposed and runtime-unreachable. | Do not contact `storage.googleapis.com` from production composition. Pagination execution remains Session 12 Batch 4. |
| OSV pagination implementation | Token-cycle controls and prefix-pass persistence | Session 12 Batch 3 defines framework-independent pagination and convergence contracts. Raw tokens remain in memory only. Token digests are in-memory cycle controls only. Pagination is not executed. | Session 12 Batch 4 implements the bounded page loop, two-pass I/O, and crash restart from page one. Production six-prefix listing remains disabled. |
