...
| OSV first-provider canary execution | Live `crates.io/` / RUSTSEC contact | Policy is Accepted as provisional technical candidate. ADR 0029 is Accepted architecture. Session 13 Batch 3C executed one operator-controlled listing-only real-provider GCS listing request (`listing_canary_one_page_classified`; one HTTPS listing request; continuation token unused; halt restored; lease released). Body retrieval remains blocked on legal revalidation. Canary-scoped operator identity adapters, the command service, the controllers, preflight, the scripted execution bridge, the provider-contact authorization service, the provider-contact persistence adapters, the listing-canary execution-authorization service, and the bounded listing-canary service exist and are unused by production runtime; OD-10 remains open for general console. Heartbeat remains required before either phase executes. | Session 13 Batch 3C-R listing-canary evidence review is mandatory. Do not retry the listing request. Do not request a second page. No activation. Listing-only before body. Aggregate body bytes ≤ 16777216. Acceptance of ADR 0029 does not authorize general OSV enablement. |
