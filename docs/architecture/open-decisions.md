...
| OD-21 | Session 9 scheduler, heartbeat, and retry policy | Bounded automatic retry is required; cadence is not. | Follow Session 8 outbox + BackgroundJob leases. Terminal runs stay historical; replay creates a new run. Batch 7B renews the BackgroundJob lease inside the synchronization service and distinguishes pre-snapshot `retry_wait` from post-snapshot job retry. No scheduler loop or BullMQ intelligence processor yet. |
