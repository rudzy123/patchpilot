...
| OD-10 | Instance operator identity | How a self-hosted admin authenticates separately from organization membership. | A config-gated bootstrap user that can manage **IntelligenceSource** rows and shared catalogs only. No cross-organization read of tenant evidence. A bypass ADR is required before any cross-org operator console. Session 13 Batch 1-R selects canary-scoped actor kind `instance_operator` with a non-tenant attestation UUID (`osv_canary_instance_operator_identity_v1`) sufficient for Batch 2A persistence. OD-10 remains open. Do not use a tenant User id. |
