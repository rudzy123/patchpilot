...
| Finding writes | Production Finding creation | **Blocked.** Finding writes are not authorized in Session 12. Gates, not a session number, are authoritative. The current roadmap places controlled Finding work no earlier than Session 16. All ADR 0026 gates remain required. ADR acceptance does not authorize writes. |
...
| OSV parser-host pending capacity | Exact pending size | ADR 0028 §10 selects **0**. Committed host constant remains `unavailable` until a later runtime-composition batch. |
| OSV retry and backoff policy | Owner, attempts, backoff, jitter, Retry-After, exhaustion | ADR 0028 §9. Session 12 Batches 5–8 persist and record retry evidence. Retry **execution** remains absent. |
| OSV scheduler and durable-job architecture | Job type `intelligence.osv.sync`, payload bounds, UTC scheduler, no catch-up | ADR 0028 §§6–7. Session 12 defines the job contract and persists request/run/lease rows. The job type is not registered. KEV `intelligence.sync` remains KEV-only. |
| OSV production observability | Bounded event codes, allowed/prohibited attributes, metric cardinality | ADR 0028 §§12–13. Session 12 Batch 9 added bounded emitters for the **disabled** composition. Production exporters remain absent. |
| OSV cleanup and retention | Evidence classes, no broad deletion, executor disabled until reviewed | ADR 0028 §14. |
| OSV first-provider canary | `crates.io/` + `rustsec_advisory_database`; exact numeric caps; no activation | ADR 0028 §15. [ADR 0029](../adr/0029-first-real-provider-osv-canary-authorization-and-safety.md) Accepted. Session 13 Batch 3C executed one operator-controlled listing-only real-provider listing request. Body retrieval remains blocked on legal revalidation. Acceptance does not authorize general OSV enablement, production synchronization, or activation. |
...
| OSV kill-switch and rollback | Halt independent from enablement; rollback is a new activation record | ADR 0028 §§17–18. Session 12 Batch 9 added typed `INTELLIGENCE_OSV_ACQUISITION_HALT`. Production composition and rollback remain later. |

## Closed in Session 12 (runtime-enablement foundation)

Session 12 implemented the disabled, uncomposed runtime-enablement foundation. Production OSV remains disabled. Session 12 Batch 10 closed that foundation. These rows are no longer open architecture questions.

| ID | Topic | Status after Session 12 |
| --- | --- | --- |
| OSV listing executor implementation | HTTPS listing adapter against ADR 0028 §1 | **Implemented and production-unreachable.** Session 12 Batch 1 `createOsvGcsListingHttpsAdapter`; Batch 2 adversarial review. Tests do not contact `storage.googleapis.com`. |
| OSV pagination implementation | Token-cycle controls and two-pass convergence | **Implemented and production-unreachable.** Session 12 Batches 3–4 and 4-R. Raw tokens remain in memory only. |
| OSV kill-switch variable | `INTELLIGENCE_OSV_ACQUISITION_HALT` | **Typed configuration implemented.** Default halted. Explicit `false` releases halt only and does not enable OSV. |
| OSV disabled runtime composition | Request/run, lease, inventory, acquisition, readiness | **Implemented and production-unreachable.** Session 12 Batch 8 `createOsvDisabledRuntimeSynchronization`; Batch 8-R review. Public factory halt-closes. Verification factory is not a public export. |
| OSV runtime-coordination persistence | Request, run, lease projection, stage attempts | **Schema and uncomposed adapters exist.** Migration `20260907120000_osv_runtime_coordination_persistence` frozen SHA-256 `7017b1c4b1d4bcae8bed4bdd0eb43559c0c89fce5b3636e0e889b276013cc3a6`. Unused by production runtime. |
| OSV bounded observability (disabled composition) | Closed events, metrics, logs, traces | **Implemented for the disabled composition.** Session 12 Batch 9 / 9-R. Event-sink failure is non-authoritative. Production exporters remain later. |

## Still open

| ID | Topic | Why it is open | Interim default for design and first implementation |
| --- | --- | --- | --- |
| OD-4 | Credential encryption key management | Envelope encryption needs an operator key, KMS, or Vault. | AES-256-GCM data keys wrapped by an operator-supplied key encryption key (KEK) loaded only through `packages/config`. No cloud KMS required for MVP. Session 13 Batch 3D-E listing-evidence encryption is a separate instance-key AES-256-GCM policy and does not close OD-4. |
...
| OD-8 | Exact outbound rate-limit and import size numbers | KEV limits are provisionally in `@patchpilot/config`. Session 11 Batch 3C compiles OSV GCS listing `maxResults` (`1000`) and listing-page byte cap (`1,048,576`). Parser, isolation, storage, retrieval-byte, and disabled-orchestration pending-work limits remain as previously closed. **ADR 0028 accepts listing pagination, token-byte (8192), canary, and production fail-closed ceilings (per-pass vs per-run arithmetic), and selects parser pending capacity 0.** Session 12 Batch 5 represents pending capacity 0 as a runtime-coordination contract. ZIP remains unselected. Committed parser-host pending-queue constant remains `unavailable` until a later runtime-composition batch. | Use the typed KEV bounds and committed OSV Session 11 constants. Use ADR 0028 numeric policies for listing and later canary work. Keep `INTELLIGENCE_OSV_ENABLED=false`. Session 12 implemented one uncomposed listing adapter and disabled composition; do not treat that as enablement, canary, or activation. |
...
| OD-10 | Instance operator identity | How a self-hosted admin authenticates separately from organization membership. | A config-gated bootstrap user that can manage **IntelligenceSource** rows and shared catalogs only. No cross-organization read of tenant evidence. A bypass ADR is required before any cross-org operator console. Session 13 Batch 2A defines canary-scoped actor kind `instance_operator` with a non-tenant attestation UUID (`osv_canary_instance_operator_identity_v1`) as a framework-independent contract. Session 13 Batch 2B persists that canary-scoped identity as schema only; Session 13 Batch 2B-R independently reviewed that schema. Session 13 Batch 2C adds uncomposed persistence adapters. Session 13 Batch 2C-R independently reviewed those adapters. Session 13 Batch 2D adds an uncomposed one-shot command that authenticates through an injected instance-operator port. Session 13 Batch 2D-R independently reviewed that command. Session 13 Batch 2E adds uncomposed heartbeat and deadline controllers and does not authenticate operators. Session 13 Batch 2E-R independently reviewed those controllers. Session 13 Batch 2F adds uncomposed executable preflight and does not authenticate operators. Session 13 Batch 2-R independently reviewed the combined chain: authentication remains identity-only and unused by production runtime. Production authentication is not operational. OD-10 remains open. Do not use a tenant User id. |
...
| OSV parser-host pending-capacity constant | Replace `unavailable` with selected `0` | ADR 0028 selects 0. Session 12 Batch 5 represents pending capacity 0 in runtime-coordination contracts. Committed isolation design still says `unavailable`. | A later runtime-composition batch may change the constant without changing occupancy-1 rejection behavior. |
| OSV retry execution | Durable job-layer retries | Session 12 Batch 5 defines `osv_runtime_retry_policy_v1` (three total attempts including the initial attempt; parser timeout max 2; bounded full jitter; HTTP 429 Retry-After capped at 30 s). Session 12 Batch 6 persists immutable stage attempts. Session 12 Batch 7 inspects retry eligibility and records exhaustion without executing retries. Session 12 Batch 7-R keeps inspection non-authoritative and reservation transactional. Session 12 Batch 8 records one-attempt disposition and does not execute retries. Session 12 Batch 8-R confirmed adapter-level retry remains absent. ADR 0029 selects **zero retries during the first real-provider canary**. No retry executor exists. | Adapters remain one-attempt. First canary stops on the first retryable provider failure. Execution remains later. |
| OSV scheduler and job-routing authorization | `intelligence.osv.sync` production registration | Session 12 Batch 5 defines the job payload and shared catalog-scope lease. Session 12 Batches 6–8 persist and compose those contracts behind a disabled factory that production startup does not import. No job type is registered. Reserved Outbox `intelligence.osv.sync.requested.v1` is deferred. Periodic heartbeat remains `deferred_to_session_13_or_dedicated_heartbeat_batch`. | Do not reuse KEV jobs. Lease fencing uses database time. Raw holder tokens are not stored. Production composition remains gated. |
| OSV production observability exporters | Production metrics, logs, and traces | Session 12 Batch 9 added bounded emitters for the disabled composition. No OpenTelemetry SDK lives in `@patchpilot/vulnerability-intelligence`. Production exporters remain absent. | Preserve the prohibition list. Do not claim exactly-once telemetry. |
...
| OSV first-provider canary execution | Live `crates.io/` / RUSTSEC contact | Policy is Accepted as provisional technical candidate. ADR 0029 is Accepted architecture. Session 13 Batch 3C executed one operator-controlled listing-only real-provider GCS listing request (`listing_canary_one_page_classified`; one HTTPS listing request; continuation token unused; halt restored; lease released). Session 13 Batch 3C-R independently reviewed that evidence. Candidate-selection metadata is unavailable; Batch 4-P is blocked. Session 13 Batch 3D-P defines protected listing-observation evidence requirements for a future separately authorized listing and does not authorize another request. Session 13 Batch 3D-P-R independently reviewed that policy. Session 13 Batch 3D-E defines the encryption policy required before schema and does not encrypt, persist, or contact a provider. Session 13 Batch 3D-E-R independently reviewed and hardened that policy; schema design may proceed. Session 13 Batch 3D-Auth implements uncomposed evidence-retaining listing authorization for a future page-one listing and does not contact a provider. Session 13 Batch 3D-Auth-R independently reviewed that authorization. Body retrieval remains blocked on legal revalidation and on a separately authorized future listing that retains protected observation metadata. Canary-scoped operator identity adapters, the command service, the controllers, preflight, the scripted execution bridge, the provider-contact authorization service, the provider-contact persistence adapters, the listing-canary execution-authorization service, the bounded listing-canary service, the Batch 3D-P evidence contracts, the Batch 3D-E encryption contracts, the Batch 3D-A persistence adapters, and the Batch 3D-Auth authorization factory exist and are unused by production runtime; OD-10 remains open for general console. Heartbeat remains required before either phase executes. | Do not retry the listing request. Do not request a second page. No activation. Listing-only before body. Aggregate body bytes ≤ 16777216. Acceptance of ADR 0029 does not authorize general OSV enablement. |
| OSV protected listing-observation evidence persistence | Durable store for listing metadata required by candidate selection | Session 13 Batch 3D-P selects Option A `durable_protected_persistence_required`. Session 13 Batch 3D-P-R independently reviewed that decision. Session 13 Batch 3D-E / 3D-E-R define and harden encryption policy `osv_protected_listing_evidence_encryption_policy_v1`. Session 13 Batch 3D-S adds schema-only tables `osv_listing_observation_evidence_set`, `osv_listing_observation_evidence`, `osv_listing_observation_evidence_envelope`, and `osv_listing_observation_evidence_purge` (`20260910120000_osv_listing_observation_evidence_persistence`, frozen SHA-256 `fd08f644fa58dc0459c43668445a7c28d5ea63e9da7fb2919f9f78055b21f654`). Session 13 Batch 3D-S-R independently reviewed and hardened that schema: legal hold blocks envelope erasure; deferred exactly-one-current envelope; `failed` is successor-only. Session 13 Batch 3D-C implements uncomposed Node.js 24 AES-256-GCM capability with synthetic proof only. Session 13 Batch 3D-C-R independently reviewed that capability. Session 13 Batch 3D-A implements uncomposed `createOsvListingObservationEvidencePersistence` (inspect-before-encrypt replay, restricted public selects, authorized protected reads, review CAS). No plaintext protected-key column. Encrypted envelopes are mandatory. Associated-data context is reconstructable from immutable columns. Purge execution remains absent. Production composition does not construct the factory. Session 13 Batch 3D-A-R independently reviewed those adapters. Session 13 Batch 3D-Auth implements uncomposed evidence-retaining listing authorization bound to those policy versions; it does not persist real protected evidence. Next checkpoint is Session 13 Batch 3D one-page protected-evidence listing canary. Batch 4-P remains blocked. | Do not persist raw listing responses or continuation tokens. Protected object keys, ciphertext, nonce, tag, and complete key identifiers are Restricted. Do not expose a bare object-key digest. Do not treat candidate eligibility as body authority. Do not destroy the shared instance key as per-set purge. |
| OSV canary catalog-generation marker | Distinguish canary candidates from production activation scope | `OsvCatalogGeneration.inventoryScope` is `osv_gcs_six_prefix_public_export_v1`. Runtime canary work scope does not mark generations canary-ineligible. | Blocking implementation requirement before bounded-body. Listing-only creates no candidate. Do not change Prisma in Session 13 Batch 2A. |
| OSV activation implementation | Explicit activation command | Adapter exists and is not invoked. Authorization is Accepted as architecture only. Candidate-age value remains an activation blocker. | After canary. Duplicate-key detection or named exception with expiry required. |
