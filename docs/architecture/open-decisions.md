...
| OSV protected listing-observation evidence persistence | Durable store for listing metadata required by candidate selection | Session 13 Batch 3D-P selects Option A `durable_protected_persistence_required` because the Batch 3C-R ephemeral database discarded candidate metadata. Session 13 Batch 3D-P-R independently reviewed that decision. Session 13 Batch 3D-E defines encryption policy `osv_protected_listing_evidence_encryption_policy_v1` (AES-256-GCM closed platform capability; associated data binds immutable context; operator-provided runtime keys later through `packages/config`; Model B controlled-redaction purge). Session 13 Batch 3D-E-R independently reviewed and hardened that policy. Future tables are named only (`osv_listing_observation_evidence_set`, `osv_listing_observation_evidence`, `osv_listing_observation_evidence_purge`). Prisma and migrations are unchanged. No plaintext protected-key column may be created, including temporarily. Next checkpoint is Session 13 Batch 3D-S. | Do not persist raw listing responses or continuation tokens. Protected object keys, ciphertext, nonce, tag, and complete key identifiers are Restricted. Do not expose a bare object-key digest. Do not treat candidate eligibility as body authority. Do not destroy the shared instance key as per-set purge. |
