...
| OSV retry implementation | Durable job-layer retries | Session 12 Batch 5 defines `osv_runtime_retry_policy_v1` (three total attempts including the initial attempt; parser timeout max 2; bounded full jitter; HTTP 429 Retry-After capped at 30 s). No retry executor exists. | Batch 6 persists attempts. Adapters remain one-attempt. |
| OSV scheduler and job implementation | `intelligence.osv.sync` and lease row | Session 12 Batch 5 defines the job payload, shared catalog-scope lease, holder/revision/fencing model, and database-time expiry as contracts. No job type is registered. No lease table exists. Reserved Outbox `intelligence.osv.sync.requested.v1` is deferred. | Batch 6 schema, then later adapters. Do not reuse KEV jobs. Lease fencing uses database time. |
