...
| OSV first-provider canary execution | Live `crates.io/` / RUSTSEC contact | Policy is Accepted as provisional technical candidate. ADR 0029 is Accepted architecture. Session 13 Batch 2A defines identity and authorization contracts. Session 13 Batch 2B adds schema-only persistence. Session 13 Batch 2B-R independently reviewed and froze that schema. Session 13 Batch 2C adds uncomposed authorization adapters. Session 13 Batch 2C-R independently reviewed those adapters. Session 13 Batch 2D adds an uncomposed one-shot command that stops at `authorized_preflight_required`. Session 13 Batch 2D-R independently reviewed that command. Not legally authorized. Legal revalidation outstanding. Canary-scoped operator identity adapters and the command service exist and are unused by production runtime; OD-10 remains open for general console. Heartbeat controller required before either phase. | Session 13 after Batch 2 operational controls, legal revalidation, configured attestation, heartbeat, runbooks, and egress proof. No activation. Listing-only before body. Aggregate body bytes ≤ 16777216. Acceptance of ADR 0029 does not authorize provider contact. Batch 2D does not authorize provider contact. |
