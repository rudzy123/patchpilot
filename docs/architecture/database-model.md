...
This is the Session 5–10 persistence design for PatchPilot. It implements the v0.1 [domain model](domain-model.md) in PostgreSQL through Prisma in `packages/database`. Session 8 Batch 4 adds graph-completeness columns, nullable Component `ecosystem`, occurrence `version_known`, insert-once graph persistence, outbox claim/execution adapters, and SQL-only CHECKs. Session 9 Batch 4C adds instance-owned KEV intelligence tables, generation-scoped entries, and PostgreSQL-only sync-run adapters. Session 10 Batch 3B applied and froze global `cve_identity` and `vulnerability_cve_identity` tables (`20260902120000_canonical_cve_identity`, SHA-256 `2190b5a0d22cf008fa01a180bc9233a68ba56159447bc599a4a2a1dba684b0ba`). Session 10 Batch 4B adds `createCveIdentityPersistence` (`identities` and `links`): insert-once ensure, unique-conflict reload, bounded batch lookup, and bounded keyset listing. Session 10 Batch 5B adds `createActiveKevMembershipPersistence`: a read-only, generation-pinned exact canonical CVE lookup against the accepted active KEV catalog. The persistent development database has eleven finished migrations. The identity and KEV persistence migrations remain **frozen**. `SbomIngestion.leaseExpiresAt` remains **unused** in Session 8.
...
| `osv_canary_instance_operator_identity`, `osv_canary_authorization` (Session 13 Batch 2B; instance-owned, unused by production runtime); `osv_canary_provider_free_preflight_attestation`, `osv_listing_provider_contact_authorization` (Session 13 Batch 3B-P; instance-owned, unused by production runtime; listing-only; no Finding relation); `osv_listing_observation_evidence_set`, `osv_listing_observation_evidence`, `osv_listing_observation_evidence_envelope`, `osv_listing_observation_evidence_purge` (Session 13 Batch 3D-S; instance-owned, unused by production runtime; encrypted identity envelopes; no plaintext protected-identity column; no Finding relation; Session 13 Batch 3D-C encrypts synthetic identities in process and does not persist envelopes or add adapters) | tenant `outbox_event`, `background_job` (when org set), `idempotency_record` |
...
`BEFORE UPDATE OR DELETE` triggers reject mutation of `audit_event`, `finding_observation`, `risk_calculation`, `vulnerability_source_record`, `evidence`, `cve_identity`, `vulnerability_cve_identity`, protected listing-observation evidence rows, and append-only purge rows. Evidence-set and envelope rows permit only accepted lifecycle transitions under dedicated mutation guards. A BEFORE INSERT trigger `patchpilot_audit_actor_membership_user` enforces that tenant audit actors match Membership user and organization. Published `risk_policy` rows cannot change identity or definition; they cannot be deleted; they may only move `published` → `retired`. Trigger functions set `search_path = pg_catalog, public` so they do not inherit the caller search path. These controls do not prevent a superuser from rewriting history.
...
[ADR 0019](../adr/0019-local-password-sessions.md) `LocalCredential` and `Session` tables, restored `actor_user_id`, and `anonymous` audit actors are in `20260827170000_audit_actor_anonymous` plus `20260827180000_local_credentials_and_sessions`. Session 5 migrations stay frozen. Session 8 graph persistence is `20260830120000_sbom_ingestion_graph_persistence` and is **frozen**; do not edit it — any SQL correction requires another forward-only migration. Session 9 KEV intelligence persistence is `20260901120000_kev_intelligence_persistence` and is **frozen**; do not edit it — any SQL correction requires another forward-only migration. Session 10 canonical CVE identity is `20260902120000_canonical_cve_identity` and is **frozen**; do not edit it — any SQL correction requires another forward-only migration. Frozen SHA-256: `2190b5a0d22cf008fa01a180bc9233a68ba56159447bc599a4a2a1dba684b0ba`. Session 13 Batch 2B adds instance-owned `osv_canary_instance_operator_identity` and `osv_canary_authorization` (`20260908120000_osv_canary_authorization_persistence`, frozen SHA-256 `321ac38a02090470aa5f09661cb0e29562327c16c9e341b44bc99516bd7fbd99`) with no tenant, Finding, or credential relation. Session 13 Batch 2B-R independently reviewed and froze that uncommitted SQL. Session 13 Batch 2C implements uncomposed PostgreSQL adapters against that frozen schema and does not add a migration. Session 13 Batch 2C-R independently reviewed those adapters. Session 13 Batch 3B-P adds instance-owned `osv_canary_provider_free_preflight_attestation` and `osv_listing_provider_contact_authorization` (`20260909120000_osv_listing_provider_contact_authorization_persistence`, frozen SHA-256 `8e9a462e329733660b970adca64fcadce0431d65f342d15dbaecf08b91a80bbc`) with no tenant, Finding, credential, token, body, URL, or JSON relation. Sixteen frozen migrations exist. Session 13 Batch 3B-P-R independently reviewed and froze that uncommitted SQL. Session 13 Batch 3B-A implements uncomposed PostgreSQL adapters against that frozen schema and does not add a migration. Session 13 Batch 3B-A-R independently reviewed those adapters. Session 13 Batch 3B-R independently reviewed the complete provider-contact authorization chain and did not change Prisma or migrations. Session 13 Batch 3C-Auth adds uncomposed listing-canary execution authorization and does not change Prisma or migrations. Session 13 Batch 3C-Auth-R independently reviewed that boundary and did not change Prisma or migrations. Session 13 Batch 3D-S adds instance-owned `osv_listing_observation_evidence_set`, `osv_listing_observation_evidence`, `osv_listing_observation_evidence_envelope`, and `osv_listing_observation_evidence_purge` (`20260910120000_osv_listing_observation_evidence_persistence`, frozen SHA-256 `fd08f644fa58dc0459c43668445a7c28d5ea63e9da7fb2919f9f78055b21f654`) with no tenant, Finding, plaintext protected-identity, continuation-token, body, URL, or JSON relation. Seventeen frozen migrations exist. Schema existence does not encrypt, persist adapters, or start canary execution. Production composition does not construct an evidence factory.
