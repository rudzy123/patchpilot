...
| Listing-token cycles | Session 12 Batch 1 listing executor exists and is uncomposed; tokens remain in-memory opaque values; ADR 0028 selects in-memory digest cycle detection and 8192-byte token ceiling. Batch 1 enforces the 8192-byte request-construction bound and does not implement cycle detection or pagination | Awaiting implementation (R3) |
