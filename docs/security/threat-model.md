...
| Listing-token cycles | Session 12 Batch 1 listing executor exists and is uncomposed; Session 12 Batch 2 adversarially reviewed single-page transport (tokens remain in-memory opaque values; 8192-byte request-construction bound). Session 12 Batch 3 defines in-memory SHA-256 token-digest cycle detection, token lineage, and crash restart from page one. Session 12 Batch 4 executes those controls in the uncomposed in-memory pagination service (one page at a time; cycles fail closed; detector state is not reused across passes). ADR 0028 selects in-memory digest cycle detection and 8192-byte token ceiling. Production listing remains uncomposed | Awaiting Session 12 Batch 4-R adversarial review; durable jobs and production composition remain later |
