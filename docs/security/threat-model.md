...
| --- | --- | --- |
| Accidental production reachability | `INTELLIGENCE_OSV_ENABLED=true` rejected; worker and API startup do not register the disabled orchestrator, listing adapter, pagination service, Batch 5 coordination contracts, Batch 7 `createOsvRuntimeCoordinationPersistence`, Batch 8 `createOsvDisabledRuntimeSynchronization`, Batch 2C `createOsvCanaryAuthorizationPersistence`, Batch 2D `createOsvCanaryOneShotCommandService`, Batch 2E `createOsvCanaryLeaseHeartbeatController`, Batch 2E `createOsvCanaryDeadlineController`, Batch 2F `createOsvCanaryPreflightService`, or Batch 3A `createOsvCanaryListingOnlyExecutionBridge`, or Batch 3B `createOsvListingProviderContactAuthorizationService`, or Batch 3B-A `createOsvListingProviderContactAuthorizationPersistence`, or Batch 3C-Auth `createOsvListingCanaryExecutionAuthorizationService`, or Batch 3C `createOsvBoundedListingCanaryService`, or Batch 3D-P-R protected listing-observation evidence constructors, or Batch 3D-C `createOsvProtectedListingEvidenceCryptographicCapability`, or Batch 3D-A `createOsvListingObservationEvidencePersistence`, or Batch 3D-Auth `createOsvEvidenceRetainingListingAuthorizationService`, or Batch 3D `createOsvProtectedEvidenceListingCanaryService`; Session 13 Batch 3C and Batch 3D operator commands are not registered in `dev`, `start`, `test`, or `build`; future job type `intelligence.osv.sync` is unregistered; ADR 0028 forbids boot wiring; `INTELLIGENCE_OSV_ACQUISITION_HALT` defaults halted | Session 12 Batch 10 confirmed production exclusion. Halt false does not enable OSV. Session 13 Batch 3C and Batch 3D executed one operator-controlled listing request each and remain unregistered from production startup. General OSV runtime remains disabled. `INTELLIGENCE_OSV_ENABLED=true` remains rejected. Session 13 Batch 3D-R independently reviews protected evidence; candidate selection remains unauthorized; Batch 4-P is blocked |
| Protected listing-observation identity leakage | Session 13 Batch 3D-P-R WeakMap-backed object identity; public results, events, logs, metrics, traces, errors, and inspect omit the raw key and the bare object-key digest; key readers are not package public exports; raw response and continuation-token retention remain prohibited. Session 13 Batch 3D encrypted 1000 retained identities with AES-256-GCM before Prisma and persisted one atomic evidence set. Public inspection omits envelope BYTEA and opaque key alias. Database access is not decryption authority. Associated data binds immutable persisted columns. Production composition does not construct the factory. | Session 13 Batch 3D-R independently reviewed this protected evidence as sufficient for deterministic candidate evaluation. Candidate selection remains unauthorized. Batch 4-P remains blocked. The process-local operator key is unavailable for later reveal. JavaScript cannot guarantee physical memory erasure. Ciphertext may remain in backups. |
