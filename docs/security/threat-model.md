...
| --- | --- | --- |
| Accidental production reachability | `INTELLIGENCE_OSV_ENABLED=true` rejected; worker and API startup do not register the disabled orchestrator, listing adapter, pagination service, Batch 5 coordination contracts, Batch 7 `createOsvRuntimeCoordinationPersistence`, or Batch 8 `createOsvDisabledRuntimeSynchronization`; the Batch 8 verification factory is not a public package export; Session 12 Batch 6 tables exist and are unused by production runtime; future job type `intelligence.osv.sync` is unregistered; ADR 0028 forbids boot wiring; `INTELLIGENCE_OSV_ACQUISITION_HALT` defaults halted | Session 12 Batch 9 added typed halt. Halt false does not enable OSV. Session 12 Batch 9-R reviewed halt parsing, public halt-port observation, and production exclusion |
