...
| --- | --- | --- |
| Accidental production reachability | `INTELLIGENCE_OSV_ENABLED=true` rejected; worker and API startup do not register the disabled orchestrator, listing adapter, pagination service, Batch 5 coordination contracts, or Batch 7 `createOsvRuntimeCoordinationPersistence`; Session 12 Batch 6 tables exist and are unused by production runtime; future job type `intelligence.osv.sync` is unregistered; ADR 0028 forbids boot wiring; future halt defaults halted | Awaiting implementation of composition (later Session 12). No enablement path in Batch 7 |
