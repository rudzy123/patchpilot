...
[ADR 0024](../adr/0024-authoritative-affected-version-source-and-osv-acquisition.md) selects OSV as the future affected-version authority and instance-owned catalog acquisition as the approved direction. The acquisition foundation is implemented and synthetically verified. Production OSV acquisition remains disabled. Session 12 implemented uncomposed listing execution, pagination, durable coordination, disabled composition, halt, and bounded observability. Production scheduler, durable OSV job routing, and automatic retry **execution** remain absent.
...
- Session 11 Batch 5F adversarially reviews that storage path. Write-once idempotency hashes stored bytes; HEAD metadata, ETags, and spoofed checksums are not content identity. Recovery is bounded and cannot mint false attached state. In-flight staged objects are not cleanup-eligible. Package boundaries keep MinIO and PostgreSQL layer tests on separate packages. Session 11 Batch 6B adds a composed MinIO-plus-PostgreSQL disabled-acquisition rehearsal in `apps/worker` integration tests using synthetic bytes and a fake retrieval port. The storage ceiling is not a provider retrieval authorization. No live provider data was used.
- Session 11 Batch 6A-P closes OD-8 provider-object retrieval at 1,048,576 bytes and defines `osv_generation_bound_retrieval_policy_v1`.
...
- Session 11 Batch 6D closes the acquisition foundation as implemented and synthetically verified. Production OSV acquisition remains disabled. Listing execution, scheduler, durable OSV jobs, and automatic retries remain absent after Session 11. Catalog activation is not invoked. Matching and Finding writes remain unauthorized. Runtime Enablement Phase R1 is Accepted [ADR 0028](../adr/0028-osv-runtime-enablement-architecture-and-safety.md). Session 12 Batch 1 implements the uncomposed listing-page HTTPS executor. Session 12 Batch 2 adversarially reviewed that executor with synthetic doubles and hardened single-page transport. That adapter does not enable OSV, paginate, or authorize activation.
- Session 12 Batch 6 adds schema-only OSV runtime coordination persistence (`20260907120000_osv_runtime_coordination_persistence`, SHA-256 `7017b1c4b1d4bcae8bed4bdd0eb43559c0c89fce5b3636e0e889b276013cc3a6`). Session 12 Batch 6-R independently reviewed that schema: holder-token digests only, separate row revision and fencing token, lease DELETE forbidden so fencing cannot reset, and no page tokens, provider bodies, tenant keys, or Findings. Session 12 Batch 7 implements uncomposed PostgreSQL adapters for those tables. Session 12 Batch 7-R reviewed those adapters. Session 12 Batch 8 composes those adapters behind an explicitly constructed disabled factory that production startup does not import. No retry executes and production composition does not construct the factory. Session 13 Batch 1-R accepts [ADR 0029](../adr/0029-first-real-provider-osv-canary-authorization-and-safety.md) for a listing-only then bounded-body canary. Session 13 Batch 2A defines instance-operator identity and single-use authorization contracts. Session 13 Batch 2B persists those contracts as schema only (`20260908120000_osv_canary_authorization_persistence`, SHA-256 `321ac38a02090470aa5f09661cb0e29562327c16c9e341b44bc99516bd7fbd99`). Session 13 Batch 2B-R independently reviewed and froze that uncommitted schema. Session 13 Batch 2C implements uncomposed PostgreSQL adapters (`createOsvCanaryAuthorizationPersistence`). Session 13 Batch 2C-R independently reviewed those adapters: revoked operators cannot issue or consume unconsumed authorizations; legal-decision and listing-review substitution fail closed; database time owns issuance, expiration, and consumption; same-run replay is status reuse, not second execution. Session 13 Batch 2D implements an uncomposed one-shot command (`createOsvCanaryOneShotCommandService`) that authenticates through an injected port, consumes one existing authorization, and stops at `authorized_preflight_required`. Session 13 Batch 2D-R independently reviewed that command: halt is re-evaluated immediately before consumption, same-run replay is status reuse, and authentication proofs remain confidential. Session 13 Batch 2E implements uncomposed `createOsvCanaryLeaseHeartbeatController` and `createOsvCanaryDeadlineController` (60000 ms cadence, 900000 ms TTL, 1800000 ms monotonic deadline). Session 13 Batch 2E-R independently reviewed those controllers: synchronous schedule callbacks fail closed; stale callbacks cannot heartbeat after reschedule; halt during in-flight work does not reschedule; already-aborted stop still awaits the in-flight heartbeat; early deadline callbacks fail closed. They do not acquire a lease, start on import, contact a provider, enable OSV, implement a production CLI, or activate a catalog. Session 13 Batch 2F implements uncomposed `createOsvCanaryPreflightService`: consumed-authorization validation, halt recheck, lease inspection without acquisition, controller-policy readiness without timers, egress policy without provider DNS or HTTP, phase-specific local dependency checks, and read-only active-pointer and zero-Finding baselines. Success is `canary_execution_preflight_passed_provider_contact_not_authorized` and is not provider authorization. Session 13 Batch 2F-R independently reviewed that preflight. Session 13 Batch 2-R independently reviewed the combined operational-control chain. Session 13 Batch 3A implements uncomposed `createOsvCanaryListingOnlyExecutionBridge` against a scripted provider port only. Session 13 Batch 3A-R independently reviewed that bridge. Session 13 Batch 3B implements uncomposed `createOsvListingProviderContactAuthorizationService`; after other gates pass it fails closed with `persistence_required` and does not mint in-memory authority. Session 13 Batch 3B-P adds schema-only distinct listing-only provider-contact authorization persistence (`20260909120000_osv_listing_provider_contact_authorization_persistence`, frozen SHA-256 `8e9a462e329733660b970adca64fcadce0431d65f342d15dbaecf08b91a80bbc`). Sixteen frozen migrations. Session 13 Batch 3B-A implements uncomposed issuance, inspection, consumption, and revocation adapters. Session 13 Batch 3C-Auth implements uncomposed listing-canary execution authorization. Session 13 Batch 3C-Auth-R independently reviewed that boundary. Session 13 Batch 3C executed one operator-controlled listing-only real-provider canary; the operator command remains unregistered from production startup. Session 13 Batch 3C-R independently reviewed that evidence and classified candidate-selection metadata as unavailable. Session 13 Batch 3D-P defines uncomposed protected listing-observation evidence policy without contacting a provider, retaining real observations, modifying Prisma, or enabling OSV. RustSec body retrieval remains provisional. Production composition does not construct the factories. OD-10 remains open for general console work. Acceptance of ADR 0029 does not authorize general OSV enablement.

## Remaining pre-runtime OSV threats

[ADR 0028](../adr/0028-osv-runtime-enablement-architecture-and-safety.md) and Accepted [ADR 0029](../adr/0029-first-real-provider-osv-canary-authorization-and-safety.md) mitigate these in architecture. Session 13 Batch 3C executed one listing-only real-provider canary. Activation, body retrieval, and general OSV enablement remain later gates. Synthetic verification does not close them.

| Threat | Existing control | Remaining gate |
| --- | --- | --- |
| Accidental production reachability | `INTELLIGENCE_OSV_ENABLED=true` rejected; worker and API startup do not register the disabled orchestrator, listing adapter, pagination service, Batch 5 coordination contracts, Batch 7 `createOsvRuntimeCoordinationPersistence`, Batch 8 `createOsvDisabledRuntimeSynchronization`, Batch 2C `createOsvCanaryAuthorizationPersistence`, Batch 2D `createOsvCanaryOneShotCommandService`, Batch 2E `createOsvCanaryLeaseHeartbeatController`, Batch 2E `createOsvCanaryDeadlineController`, Batch 2F `createOsvCanaryPreflightService`, or Batch 3A `createOsvCanaryListingOnlyExecutionBridge`, or Batch 3B `createOsvListingProviderContactAuthorizationService`, or Batch 3B-A `createOsvListingProviderContactAuthorizationPersistence`, or Batch 3C-Auth `createOsvListingCanaryExecutionAuthorizationService`, or Batch 3C `createOsvBoundedListingCanaryService`, or Batch 3D-P-R protected listing-observation evidence constructors, or Batch 3D-C `createOsvProtectedListingEvidenceCryptographicCapability`, or Batch 3D-A `createOsvListingObservationEvidencePersistence`, or Batch 3D-Auth `createOsvEvidenceRetainingListingAuthorizationService`; Session 13 Batch 3D-Auth-R independently reviewed that authorization; Session 13 Batch 2F-R independently reviewed that preflight; Session 13 Batch 2-R independently reviewed the combined chain; the Batch 8 verification factory is not a public package export; the Batch 3A scripted listing-capability constructor is not a public package export; Session 13 Batch 3A-R independently reviewed that bridge; Session 12 Batch 6 tables exist and are unused by production runtime; Session 13 Batch 2B canary authorization tables exist and are unused by production runtime; Session 13 Batch 3B-P provider-contact authorization tables exist and are unused by production runtime; future job type `intelligence.osv.sync` is unregistered; ADR 0028 forbids boot wiring; `INTELLIGENCE_OSV_ACQUISITION_HALT` defaults halted; ADR 0029 keeps production services halted and requires durable one-shot canary authorization; Session 13 Batch 2C adapters do not issue or consume authorization at startup; Session 13 Batch 2D-R does not authenticate or consume authorization on import; Session 13 Batch 2E-R does not start timers on import or construction; Session 13 Batch 2F does not execute preflight on import; Session 13 Batch 3A does not execute the bridge on import; Session 13 Batch 3B does not execute or issue durable provider-contact authorization on import; Session 13 Batch 3B-A adapters exist, were independently reviewed in Session 13 Batch 3B-A-R, and are unused by production runtime; Session 13 Batch 3B-R independently reviewed the complete authorization chain; Session 13 Batch 3C-Auth does not acquire a lease, start heartbeat, or create a permit on import; Session 13 Batch 3C-Auth-R independently reviewed that boundary; Session 13 Batch 3C operator command is not registered in `dev`, `start`, `test`, or `build`; Session 13 Batch 3D-P evidence contracts are exported and unused by production startup; Session 13 Batch 3D-E encryption contracts are exported and unused by production startup and do not encrypt, decrypt, or persist; Session 13 Batch 3D-A persistence adapters exist and are unused by production runtime | Session 12 Batch 10 confirmed production exclusion. Halt false does not enable OSV. Session 13 Batch 3C executed one operator-controlled listing-only real-provider request and remains unregistered from production startup. Session 13 Batch 3D-P is provider-free policy only. Session 13 Batch 3D-E is encryption policy only. General OSV runtime remains disabled. `INTELLIGENCE_OSV_ENABLED=true` remains rejected. Session 13 Batch 3C-R independently reviewed that listing canary; candidate-selection evidence is unavailable; Batch 4-P is blocked |
| Protected listing-observation identity leakage | Session 13 Batch 3D-P-R WeakMap-backed object identity; public results, events, logs, metrics, traces, errors, and inspect omit the raw key and the bare object-key digest; key readers are not package public exports; raw response and continuation-token retention remain prohibited; tests use synthetic markers only. Session 13 Batch 3D-E requires authenticated AES-256-GCM envelopes, associated-data binding so transplanted ciphertext fails, no hardcoded keys, no plaintext protected-key column, public contracts omitting ciphertext, nonce, tag, and raw key identifiers, and Model B envelope redaction rather than shared-key destruction. Session 13 Batch 3D-S persists those envelopes as restricted BYTEA with reconstructable associated-data context and no plaintext identity column. Session 13 Batch 3D-S-R independently reviewed that schema: legal hold blocks envelope erasure; rotation cannot destroy the only readable envelope; public repositories do not select envelope fields | Session 13 Batch 3D-A persistence adapters keep the encrypted envelope Restricted and omit BYTEA and opaque key alias from public selects. Protected reads require a closed reveal purpose and return process-local identity only. Database access is not decryption authority. Session 13 Batch 3D-C implements uncomposed Node.js 24 AES-256-GCM with operator-supplied runtime keys unused by startup; JavaScript cannot guarantee physical memory erasure. Session 13 Batch 3D-C-R independently reviewed that capability. Production composition does not construct the factory. Session 13 Batch 3D-A-R independently reviewed the persistence adapters. Session 13 Batch 3D-Auth implements uncomposed evidence-retaining listing authorization and does not load a production key, persist real protected evidence, or contact a provider. Session 13 Batch 3D-Auth-R independently reviewed that authorization. Batch 4-P remains blocked |
| Listing-token cycles | Session 12 Batch 1 listing executor exists and is uncomposed; Session 12 Batch 2 adversarially reviewed single-page transport (tokens remain in-memory opaque values; 8192-byte request-construction bound). Session 12 Batch 3 defines in-memory SHA-256 token-digest cycle detection, token lineage, and crash restart from page one. Session 12 Batch 4 executes those controls in the uncomposed in-memory pagination service (one page at a time; cycles fail closed; detector state is not reused across passes). Session 12 Batch 4-R adversarially reviewed and hardened the page loop, token-scope isolation, atomic admission, and hung-port cancellation. ADR 0028 selects in-memory digest cycle detection and 8192-byte token ceiling. Production listing remains uncomposed | Durable job registration and production composition remain later. Batch 6/7 do not persist page tokens or token digests |
| Retry storms | One-attempt retrieval; orchestrator records retry disposition only; Session 12 Batch 5 contracts cap 3 total attempts including the initial attempt, parser timeout max 2, bounded full jitter, and HTTP 429 Retry-After at 30 s; Session 12 Batch 6 CHECKs reject ordinal 4; Session 12 Batch 7 records exhaustion and inspects eligibility without executing retries; Session 12 Batch 7-R keeps inspection non-authoritative and reservation ordered | Awaiting executor (later composition) |
| Incomplete-catalog activation | Integer reconciliation, blocking quarantine, `ready_for_activation` gate; disabled orchestration never activates; ADR 0028 requires explicit activation | Activation blocking (after Session 13 canary) |
...
| Operational data leakage | Confidential failure taxonomy omits bodies, keys, URLs, and tenant data; Session 12 Batch 5 and Batch 9 operational-event contracts omit holder tokens, page tokens, bodies, URLs, headers, SQL, tenants, and Findings; Batch 9 added bounded emitters for the disabled composition | Production exporters remain later. Do not claim exactly-once telemetry |
