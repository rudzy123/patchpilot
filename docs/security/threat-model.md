...
| --- | --- | --- |
| Accidental production reachability | `INTELLIGENCE_OSV_ENABLED=true` rejected; worker and API startup do not register the disabled orchestrator, listing adapter, pagination service, Batch 5 coordination contracts, Batch 7 `createOsvRuntimeCoordinationPersistence`, Batch 8 `createOsvDisabledRuntimeSynchronization`, Batch 2C `createOsvCanaryAuthorizationPersistence`, or Batch 2D `createOsvCanaryOneShotCommandService`; the Batch 8 verification factory is not a public package export; Session 12 Batch 6 tables exist and are unused by production runtime; Session 13 Batch 2B canary authorization tables exist and are unused by production runtime; future job type `intelligence.osv.sync` is unregistered; ADR 0028 forbids boot wiring; `INTELLIGENCE_OSV_ACQUISITION_HALT` defaults halted; ADR 0029 keeps production services halted and requires durable one-shot canary authorization; Session 13 Batch 2C adapters do not issue or consume authorization at startup; Session 13 Batch 2D-R does not authenticate or consume authorization on import | Session 12 Batch 10 confirmed production exclusion. Halt false does not enable OSV. Session 13 Batch 2D-R does not register production OSV routing. Canary execution remains unauthorized |
