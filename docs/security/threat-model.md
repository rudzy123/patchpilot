...
| Listing-token cycles | Session 12 Batch 1 listing executor exists and is uncomposed; Session 12 Batch 2 adversarially reviewed single-page transport (tokens remain in-memory opaque values; no digest store). ADR 0028 selects in-memory digest cycle detection and 8192-byte token ceiling. Batch 1/2 enforce the 8192-byte request-construction bound and do not implement cycle detection or pagination | Awaiting implementation (R3) |
