...
| --- | --- | --- |
| Accidental production reachability | `INTELLIGENCE_OSV_ENABLED=true` rejected; worker and API startup do not register the disabled orchestrator, listing adapter, pagination service, Batch 5 coordination contracts, Batch 7 `createOsvRuntimeCoordinationPersistence`, or Batch 8 `createOsvDisabledRuntimeSynchronization`; the Batch 8 verification factory is not a public package export; Session 12 Batch 6 tables exist and are unused by production runtime; future job type `intelligence.osv.sync` is unregistered; ADR 0028 forbids boot wiring; future halt defaults halted | Session 12 Batch 8-R reviewed composition reachability. No enablement path. Session 12 Batch 9 kill switch remains later |
...
| Operational data leakage | Confidential failure taxonomy omits bodies, keys, URLs, and tenant data; Session 12 Batch 5 operational-event contracts omit holder tokens, page tokens, bodies, URLs, headers, SQL, tenants, and Findings | Awaiting emitters (later runtime batch) |
...
| Rollback and kill switch | ADR 0028 defines halt independent from enablement and rollback as a new activation record. Session 12 Batch 5 contracts default halt to halted. Variable not added | Awaiting implementation (later composition for halt variable, R7 rollback) |
