...
| --- | --- | --- |
| Accidental production reachability | `INTELLIGENCE_OSV_ENABLED=true` rejected; worker and API startup do not register the disabled orchestrator, listing adapter, pagination service, Batch 5 coordination contracts, Batch 7 `createOsvRuntimeCoordinationPersistence`, Batch 8 `createOsvDisabledRuntimeSynchronization`, Batch 2C `createOsvCanaryAuthorizationPersistence`, Batch 2D `createOsvCanaryOneShotCommandService`, Batch 2E `createOsvCanaryLeaseHeartbeatController`, Batch 2E `createOsvCanaryDeadlineController`, Batch 2F `createOsvCanaryPreflightService`, or Batch 3A `createOsvCanaryListingOnlyExecutionBridge`, or Batch 3B `createOsvListingProviderContactAuthorizationService`, or Batch 3B-A `createOsvListingProviderContactAuthorizationPersistence`, or Batch 3C-Auth `createOsvListingCanaryExecutionAuthorizationService`, or Batch 3C `createOsvBoundedListingCanaryService`, or Batch 3D-P-R protected listing-observation evidence constructors, or Batch 3D-E encryption-policy constructors; Session 13 Batch 2F-R independently reviewed that preflight; Session 13 Batch 2-R independently reviewed the combined chain; the Batch 8 verification factory is not a public package export; the Batch 3A scripted listing-capability constructor is not a public package export; Session 13 Batch 3A-R independently reviewed that bridge; Session 12 Batch 6 tables exist and are unused by production runtime; Session 13 Batch 2B canary authorization tables exist and are unused by production runtime; Session 13 Batch 3B-P provider-contact authorization tables exist and are unused by production runtime; future job type `intelligence.osv.sync` is unregistered; ADR 0028 forbids boot wiring; `INTELLIGENCE_OSV_ACQUISITION_HALT` defaults halted; ADR 0029 keeps production services halted and requires durable one-shot canary authorization; Session 13 Batch 2C adapters do not issue or consume authorization at startup; Session 13 Batch 2D-R does not authenticate or consume authorization on import; Session 13 Batch 2E-R does not start timers on import or construction; Session 13 Batch 2F does not execute preflight on import; Session 13 Batch 3A does not execute the bridge on import; Session 13 Batch 3B does not execute or issue durable provider-contact authorization on import; Session 13 Batch 3B-A adapters exist, were independently reviewed in Session 13 Batch 3B-A-R, and are unused by production runtime; Session 13 Batch 3B-R independently reviewed the complete authorization chain; Session 13 Batch 3C-Auth does not acquire a lease, start heartbeat, or create a permit on import; Session 13 Batch 3C-Auth-R independently reviewed that boundary; Session 13 Batch 3C operator command is not registered in `dev`, `start`, `test`, or `build`; Session 13 Batch 3D-P evidence contracts are exported and unused by production startup; Session 13 Batch 3D-E encryption contracts are exported and unused by production startup and do not encrypt, decrypt, or persist | Session 12 Batch 10 confirmed production exclusion. Halt false does not enable OSV. Session 13 Batch 3C executed one operator-controlled listing-only real-provider request and remains unregistered from production startup. Session 13 Batch 3D-P is provider-free policy only. Session 13 Batch 3D-E is encryption policy only. General OSV runtime remains disabled. `INTELLIGENCE_OSV_ENABLED=true` remains rejected. Session 13 Batch 3C-R independently reviewed that listing canary; candidate-selection evidence is unavailable; Batch 4-P is blocked |
| Protected listing-observation identity leakage | Session 13 Batch 3D-P-R WeakMap-backed object identity; public results, events, logs, metrics, traces, errors, and inspect omit the raw key and the bare object-key digest; key readers are not package public exports; raw response and continuation-token retention remain prohibited; tests use synthetic markers only. Session 13 Batch 3D-E requires authenticated AES-256-GCM envelopes, associated-data binding so transplanted ciphertext fails, no hardcoded keys, no plaintext protected-key column, public contracts omitting ciphertext, nonce, tag, and raw key identifiers, and Model B envelope redaction rather than shared-key destruction. Session 13 Batch 3D-S persists those envelopes as restricted BYTEA with reconstructable associated-data context and no plaintext identity column. Session 13 Batch 3D-S-R independently reviewed that schema: legal hold blocks envelope erasure; rotation cannot destroy the only readable envelope; public repositories do not select envelope fields | Future persistence adapters must keep the encrypted envelope Restricted, not publicly selected, and must not create a plaintext object-key column. Public queries must omit envelope columns and opaque key aliases. Database access is not decryption authority. Session 13 Batch 3D-C implements uncomposed Node.js 24 AES-256-GCM with operator-supplied runtime keys unused by startup; JavaScript cannot guarantee physical memory erasure. Session 13 Batch 3D-C-R independently reviewed that capability. Batch 3D-A is next. Batch 4-P remains blocked |
