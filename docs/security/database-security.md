...
- Session 13 Batch 3D-E forbids a plaintext protected listing-object-key column. Session 13 Batch 3D-S stores an authenticated ciphertext envelope as restricted `nonce_12 || ciphertext || tag_16` BYTEA, not reversible encoding or a plaintext transitional column. Raw encryption keys must not appear in database rows. Opaque key aliases are stored and are not indexed. Session 13 Batch 3D-E-R accepted that envelope contract. Session 13 Batch 3D-S-R independently reviewed the schema: legal hold blocks envelope erasure; deferred exactly-one-current envelope; no public repository selects envelope fields. Session 13 Batch 3D-C encrypts synthetic identities in process using Node.js 24 AES-256-GCM. Session 13 Batch 3D-C-R independently reviewed that capability. Session 13 Batch 3D-A implements uncomposed persistence adapters that persist restricted envelopes and omit BYTEA and opaque key alias from public selects. Session 13 Batch 3D-A-R independently reviewed those adapters: inspect-before-encrypt replay, associated-data equivalence, request/run-bound protected reads, review CAS, and legal-hold CAS. Production composition does not construct the factory. Purge execution remains absent. Session 13 Batch 3D-Auth adds no Prisma or migration change and does not persist real protected evidence. Session 13 Batch 3D-Auth-R independently reviewed that authorization and did not modify Prisma.
