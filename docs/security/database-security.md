...
- Session 13 Batch 3D-E forbids a plaintext protected listing-object-key column. Future listing-evidence rows, if added in Batch 3D-S, must store an authenticated ciphertext envelope, not reversible encoding. Raw encryption keys must not appear in database rows. Session 13 Batch 3D-E-R accepted that envelope contract.
