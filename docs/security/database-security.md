...
- Session 13 Batch 3D-E forbids a plaintext protected listing-object-key column. Session 13 Batch 3D-S stores an authenticated ciphertext envelope as restricted `nonce_12 || ciphertext || tag_16` BYTEA. Session 13 Batch 3D persisted 1000 such envelopes as one atomic `review_pending` evidence set through uncomposed adapters. Public selects omit envelope BYTEA and opaque key alias. No plaintext provider identity entered Prisma. Production composition does not construct the factory. Purge execution remains absent. Session 13 Batch 3D-R independently reviewed this evidence.
