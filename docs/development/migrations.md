...
13. Session 12 Batch 6 / 6-R OSV runtime coordination persistence is `20260907120000_osv_runtime_coordination_persistence`. Batch 6 creates Prisma models and this migration. Batch 6-R reviewed and hardened the uncommitted SQL before freeze. Frozen SHA-256: `7017b1c4b1d4bcae8bed4bdd0eb43559c0c89fce5b3636e0e889b276013cc3a6`. Do not edit it. Any SQL correction requires another forward-only migration. No lease, retry, or scheduler adapter is included. No seed rows. No tenant or Finding relation. Lease projection DELETE is forbidden so fencing tokens cannot reset. Digest columns are TEXT with exact lowercase SHA-256 CHECKs so CHAR/VARCHAR(64) trailing-space truncation cannot admit a padded value.
14. Do not edit Session 3, Session 5, Session 6, Session 7, Session 8, Session 9, Session 10, Session 11, Session 12 Batch 6, or the committed correction migration files. Those SQL files are the authoritative extras (checks, partial unique indexes, triggers). There is no separately applied `prisma/sql/*.sql` extras source.
15. There is no down migration. Rollback is restore-from-backup or a **forward-fix** migration.

## Paths

### Clean database

`pnpm db:migrate:deploy` on an empty database applies Session 3, Session 5, the Session 5 corrective migrations, Session 6 authentication persistence, Session 7 asset inventory constraints, Session 8 graph persistence, Session 9 KEV intelligence persistence, Session 10 canonical CVE identity, Session 11 OSV acquisition persistence, the Session 11 parsed-revision ID CHECK correction, then Session 12 Batch 6 OSV runtime coordination persistence. Isolated migration tests use that fourteen-migration sequence. The persistent development database has those fourteen frozen migrations, including `20260907120000_osv_runtime_coordination_persistence`. The placeholder table exists only between Session 3 and Session 5.
...
A database that already has `20260901120000_kev_intelligence_persistence` applies `20260902120000_canonical_cve_identity`, `20260904120000_osv_acquisition_persistence_foundation`, `20260904180000_osv_parsed_revision_id_check_correction`, then `20260907120000_osv_runtime_coordination_persistence`. The Session 10 migration creates two global append-only identity tables. The Session 11 Batch 5C migration adds OSV acquisition tables only. Batch 5C-R replaces only the parsed OSV ID CHECK. Batch 6 adds OSV runtime coordination tables only. None rewrite Vulnerability or Finding rows. No active OSV catalog is seeded. Isolated tests cover this path. Do not reset the database or delete Docker volumes.

### Upgrade from Session 10

A database that already has `20260902120000_canonical_cve_identity` applies `20260904120000_osv_acquisition_persistence_foundation`, `20260904180000_osv_parsed_revision_id_check_correction`, then `20260907120000_osv_runtime_coordination_persistence`. The Batch 5C migration adds global OSV acquisition tables and does not rewrite tenant, Finding, KEV, or canonical CVE identity rows. Batch 5C-R changes no data. Batch 6 adds runtime coordination tables only and seeds no rows. No active OSV catalog is seeded. Isolated tests cover this path. Do not reset the database or delete Docker volumes.

### Upgrade from Session 11 Batch 5C

A database that already has `20260904120000_osv_acquisition_persistence_foundation` applies `20260904180000_osv_parsed_revision_id_check_correction` then `20260907120000_osv_runtime_coordination_persistence`. Batch 5C-R drops and recreates `osv_parsed_advisory_revision_osv_id_chk`. Batch 6 adds OSV runtime coordination tables only. They do not rewrite existing rows. Isolated tests cover this path. Do not reset the database or delete Docker volumes.

### Upgrade from Session 11 Batch 5C-R

A database that already has `20260904180000_osv_parsed_revision_id_check_correction` applies only `20260907120000_osv_runtime_coordination_persistence`. That migration adds OSV runtime coordination tables only. It does not rewrite tenant, Finding, KEV, canonical CVE identity, or OSV acquisition rows. No lease, request, run, or attempt row is seeded. Isolated tests cover this path. Do not reset the database or delete Docker volumes.
