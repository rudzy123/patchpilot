...
| `INTELLIGENCE_OSV_ACQUISITION_HALT` | OSV acquisition emergency halt. Default `true` (halted). Missing, empty, and whitespace-only values are halted. Own properties only; prototype values cannot supply the setting. Exact `true`/`false` after trim. Malformed values fail validation. Explicit `false` releases halt only and does not enable OSV, start a scheduler, or contact a provider. Refresh is a process snapshot; restart is required. Session 13 Batch 2C-R does not make halt false a canary trigger: production worker/API remain halted; authorization adapters exist and are unused by production runtime; a future one-shot command would require durable canary authorization plus command-local halt only. |
