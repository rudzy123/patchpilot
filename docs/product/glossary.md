...
| **Finding** | Tenant-owned link between an **asset**, a **versionless** **Component**, and one **Vulnerability** advisory. Natural key: `organizationId` + `assetId` + `componentId` + `vulnerabilityId` ([ADR 0026](../adr/0026-authoritative-match-evidence-and-finding-lifecycle.md)). The key is the advisory UUID, not the OSV id string, CVE, CveIdentity, occurrence, SBOM, ingestion, provider revision, or package version. Spans ingestions. Per-ingestion presence is a **Finding observation**, not a new finding when identity matches. Generic Finding persistence exists; Session 9, Session 11, and Session 12 must not create or modify Findings. |
...
| **Intelligence source** | Global OSV/CISA KEV synchronization state. Not a tenant installation. CISA KEV scheduled import exists in the worker; authenticated provider-status GET routes exist (`intelligence:read`). The Session 12 OSV runtime-enablement foundation exists and remains production-disabled. ZIP processing, matching, Findings, a dashboard, and manual sync/retry are not implemented. |
