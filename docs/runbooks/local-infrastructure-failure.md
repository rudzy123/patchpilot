...
3. From the repository root: `pnpm infrastructure:status` (container names, ports, and health). Then `pnpm infrastructure:logs` if a service is not healthy. After `pnpm infrastructure:down`, `pnpm infrastructure:status` prints an empty table and exits 0 because no containers are running.
